# 触发器与效果

## 触发器
### 什么是触发# 触发器与效果
### 什么是触发器？
触发器用作条件语句# 触发器与效果
触发器用作条件语句，它们只评估当前游戏状态并在每次评估时返回 `TRUE` 或 `FALSE`，# 触发器与效果
触发器用作条件语句，它们只评估当前游戏状态并在每次评估时返回 `TRUE` 或 `FALSE`，可以把它们想象成在提问。
### 触发器字节码
触发器被存储为字节码，然后可以进行优化以减少内部# 触发器与效果
触发器被存储为字节码，然后可以进行优化以减少内部字节码大小。
所有字节码都存储在 16 位单元中。了解# 触发器与效果
所有字节码都存储在 16 位单元中。了解第一个作用域/指令的起始位置很重要，因为字节码是位置敏感的。
例如# 触发器与效果
例如，考虑以下触发器字节码序列：
| 位置 | 数据 |
|---|---|
| 0 | `trigger::# 触发器与效果
| 0 | `trigger::generic_scope` |
| 1 | `1` |
这描述了一个有效载荷大小为 1 的作用域。有效载荷# 触发器与效果
这描述了一个有效载荷大小为 1 的作用域。有效载荷包括作用域内的所有内容加上有效载荷本身的大小。
| 位置 | 数据# 触发器与效果
| 0 | `# 触发器与效果
| 0 | `trigger::is_disjunctive_scope, trigger::# 触发器与效果
| 0 | `trigger::is_disjunctive_scope, trigger::generic_scope` |
| 1 | `# 触发器与效果
| 1 | `8` |
| 2 | `trigger# 触发器与效果
| 2 | `trigger::association_ne, trigger::owns` |
| 3 | `100` |
| 4 | `trigger::x_core_scope_n# 触发器与效果
| 4 | `trigger::x_core_scope_nation` |
| 5 | `4# 触发器与效果
| 5 | `4` |
| 6 | `trigger::# 触发器与效果
| 6 | `trigger::association_lt, trigger::blockade` |
| 7 | `trigger::payload(float(# 触发器与效果
| 7 | `trigger::payload(float(2.0))` |
这大致# 触发器与效果
这大致等效于以下代码：
```sh
OR = { #Disjuntive scope
    owns != 100 #Does not own# 触发器与效果
    owns != 100 #Does not own province 100
    all_core = { ## 触发器与效果
    all_core = { #x_core_scope_nation
        blockade < 1 #blockade is < 1
    }
}
```
`trigger# 触发器与效果
`trigger::is_disjunctive_scope` 告诉我们正在# 触发器与效果
`trigger::is_disjunctive_scope` 告诉我们正在处理一个作用域，其中我们只需选择一条# 触发器与效果
`trigger::is_disjunctive_scope` 告诉我们正在处理一个作用域，其中我们只需选择一条路径（与 AND 选择所有路径相反）。# 触发器与效果
`trigger::is_disjunctive_scope` 告诉我们正在处理一个作用域，其中我们只需选择一条路径（与 AND 选择所有路径相反）。`trigger::generic_scope` 将此 16# 触发器与效果
`trigger::is_disjunctive_scope` 告诉我们正在处理一个作用域，其中我们只需选择一条路径（与 AND 选择所有路径相反）。`trigger::generic_scope` 将此 16 位单元标记为属于一个作用域。这意味着下一个单元包含有效载荷大小。在本例中为 8 个单元（包括有效载荷大小本身），这是正确的。现在作用域内的每个子项都需要单独解析——这使得字节码非常脆弱，因此务必仔细检查是否从有效位置# 触发器与效果
`trigger::is_disjunctive_scope` 告诉我们正在处理一个作用域，其中我们只需选择一条路径（与 AND 选择所有路径相反）。`trigger::generic_scope` 将此 16 位单元标记为属于一个作用域。这意味着下一个单元包含有效载荷大小。在本例中为 8 个单元（包括有效载荷大小本身），这是正确的。现在作用域内的每个子项都需要单独解析——这使得字节码非常脆弱，因此务必仔细检查是否从有效位置读取。
`trigger::owns` 是在问"我们拥有以下内容吗？"，然后 `trigger::association_ne` 将# 触发器与效果
`trigger::owns` 是在问"我们拥有以下内容吗？"，然后 `trigger::association_ne` 将条件问题否定为"我们**不**拥有以下内容吗？"。其中 `100`# 触发器与效果
`trigger::owns` 是在问"我们拥有以下内容吗？"，然后 `trigger::association_ne` 将条件问题否定为"我们**不**拥有以下内容吗？"。其中 `100` 是省份的 Id。为了获得非作用域# 触发器与效果
`trigger::owns` 是在问"我们拥有以下内容吗？"，然后 `trigger::association_ne` 将条件问题否定为"我们**不**拥有以下内容吗？"。其中 `100` 是省份的 Id。为了获得非作用域字节码指令的固定大小，使用了一个查找# 触发器与效果
`trigger::owns` 是在问"我们拥有以下内容吗？"，然后 `trigger::association_ne` 将条件问题否定为"我们**不**拥有以下内容吗？"。其中 `100` 是省份的 Id。为了获得非作用域字节码指令的固定大小，使用了一个查找表。
随后，我们看到 `trigger::# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association_lt` "小于" 与 `trigger::blockade` "该省份被封锁了吗？# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association_lt` "小于" 与 `trigger::blockade` "该省份被封锁了吗？" 进行 OR 运算，形成问题：# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association_lt` "小于" 与 `trigger::blockade` "该省份被封锁了吗？" 进行 OR 运算，形成问题："封锁值是否小于 x？"，其中# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association_lt` "小于" 与 `trigger::blockade` "该省份被封锁了吗？" 进行 OR 运算，形成问题："封锁值是否小于 x？"，其中 `x` 对应第一个单元，该单元# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association_lt` "小于" 与 `trigger::blockade` "该省份被封锁了吗？" 进行 OR 运算，形成问题："封锁值是否小于 x？"，其中 `x` 对应第一个单元，该单元通过 `trigger::payload(float(2.0# 触发器与效果
随后，我们看到 `trigger::x_core_scope_nation`，对应于 `all_core = { ... }`（`any_core` 有一个额外的 OR 运算的 `trigger::is_existence_scope`）。接下来是有效载荷大小，为 4，这意味着总数据（加上有效载荷）大小为 4。然后我们得到第一个条件，即 `trigger::association_lt` "小于" 与 `trigger::blockade` "该省份被封锁了吗？" 进行 OR 运算，形成问题："封锁值是否小于 x？"，其中 `x` 对应第一个单元，该单元通过 `trigger::payload(float(2.0))` 从浮点数编码为 uint16_t。
### 打包浮点数
有时你可能会看到触发器的有效载荷大小为# 触发器与效果
有时你可能会看到触发器的有效载荷大小为 2（如下表所示），但它只有 1 个参数，在修复所谓的 bug 之前，请注意这是一个有意的设计。浮点数存储# 触发器与效果
有时你可能会看到触发器的有效载荷大小为 2（如下表所示），但它只有 1 个参数，在修复所谓的 bug 之前，请注意这是一个有意的设计。浮点数存储为两个连续的 `uint16_t` 单元# 触发器与效果
有时你可能会看到触发器的有效载荷大小为 2（如下表所示），但它只有 1 个参数，在修复所谓的 bug 之前，请注意这是一个有意的设计。浮点数存储为两个连续的 `uint16_t` 单元，通过将它们打包到联合体中并以二进制形式读回来组成。
- `void add# 触发器与效果
- `void add_float_to_payload(float f)` ：允许写入浮# 触发器与效果
- `void add_float_to_payload(float f)` ：允许写入浮点有效载荷。
- `float read_float_from_payload(uint16_t const* data)` ：# 触发器与效果
- `float read_float_from_payload(uint16_t const* data)` ：允许读回浮点有效载荷。
### 触发器列表
这不是详尽的触发器列表，请注意触发器代码**可能**带有标志，因此正确解析它们非常重要。# 触发器与效果
这不是详尽的触发器列表，请注意触发器代码**可能**带有标志，因此正确解析它们非常重要。这很简单，只需执行按位与操作，如下所示：`value & trigger::code_mask`# 触发器与效果
这不是详尽的触发器列表，请注意触发器代码**可能**带有标志，因此正确解析它们非常重要。这很简单，只需执行按位与操作，如下所示：`value & trigger::code_mask`（其中 `value` 是单元格的 `# 触发器与效果
这不是详尽的触发器列表，请注意触发器代码**可能**带有标志，因此正确解析它们非常重要。这很简单，只需执行按位与操作，如下所示：`value & trigger::code_mask`（其中 `value` 是单元格的 `uint16_t` 值，被认为是触发器的代码）。触发器仅评估
| 代码 | 名称 | 参数数量 |# 触发器与效果
| 代码 | 名称 | 参数数量 | 备注/参数 |
|---|---|---|---|
|# 触发器与效果
| 0x0000 | none | 0# 触发器与效果
| 0x0000 | none | 0 | Empty, dummy trigger that does nothing |
| 0x0001 | year |# 触发器与效果
| 0x0001 | year | 1 | Compares to the year #1 (`uint16_t`) |
| 0x0002 | month | 1 | Compares# 触发器与效果
| 0x0002 | month | 1 | Compares the current month to #1 (`uint16_t`) |
| 0x0003 |# 触发器与效果
| 0x0003 | port | 0 | Does this `dcon# 触发器与效果
| 0x0003 | port | 0 | Does this `dcon::province_id` have a port? |
| 0x0004 | rank |# 触发器与效果
| 0x0004 | rank | 1 | Compares the rank of the primary `# 触发器与效果
| 0x0004 | rank | 1 | Compares the rank of the primary `dcon::nation_id` to #1# 触发器与效果
| 0x0004 | rank | 1 | Compares the rank of the primary `dcon::nation_id` to #1 (`uint16_t`) |
| 0# 触发器与效果
| 0x0005 | technology | 1 | Checks# 触发器与效果
| 0x0005 | technology | 1 | Checks that primary `dcon::nation_id` has technology #1 (`dcon::technology_id# 触发器与效果
| 0x0005 | technology | 1 | Checks that primary `dcon::nation_id` has technology #1 (`dcon::technology_id`) |
| 0x0006 | strata_rich | 0 | Is the primary# 触发器与效果
| 0x0006 | strata_rich | 0 | Is the primary `dcon::pop_id` part of the# 触发器与效果
| 0x0006 | strata_rich | 0 | Is the primary `dcon::pop_id` part of the rich strata? |
| 0x000# 触发器与效果
| 0x0007 | life_rating_province | 1 |# 触发器与效果
| 0x0007 | life_rating_province | 1 | Checks the liferating of the primary `d# 触发器与效果
| 0x0007 | life_rating_province | 1 | Checks the liferating of the primary `dcon::province_id` with #1 (`uint# 触发器与效果
| 0x0007 | life_rating_province | 1 | Checks the liferating of the primary `dcon::province_id` with #1 (`uint16_t`) |
| 0x0008 | life_rating_state | 1 | Checks the liferating of the primary `dcon# 触发器与效果
| 0x0008 | life_rating_state | 1 | Checks the liferating of the primary `dcon::state_instance_id` with #1 (`uint16_t`) |
| 0x0009 | has_empty_adjacent_state_province | 0 | Does the primary `dcon::# 触发器与效果
| 0x0009 | has_empty_adjacent_state_province | 0 | Does the primary `dcon::province_id` have an empty adjacent state? |
| 0x000A | has_empty# 触发器与效果
| 0x000A | has_empty_adjacent_state_state | 0 | Does the primary `dcon::state_instance_id` have# 触发器与效果
| 0x000A | has_empty_adjacent_state_state | 0 | Does the primary `dcon::state_instance_id` have an empty adjacent state? |
| 0x000B | state_id_province |# 触发器与效果
| 0x000B | state_id_province | 1 | Does the primary `dcon::province_id` pertain to the same state as #1 (`dcon::province_id`)? |
| 0x000C | state_id# 触发器与效果
| 0x000C | state_id_state | 1 | Does the primary `d# 触发器与效果
| 0x000C | state_id_state | 1 | Does the primary `dcon::state_instance_id` pertain to the# 触发器与效果
| 0x000C | state_id_state | 1 | Does the primary `dcon::state_instance_id` pertain to the same state as #1 (`dcon::province# 触发器与效果
| 0x000C | state_id_state | 1 | Does the primary `dcon::state_instance_id` pertain to the same state as #1 (`dcon::province_id`)? |
| 0x000D | cash_reserves | 2 | Tests# 触发器与效果
| 0x000D | cash_reserves | 2 | Tests that the primary `dcon::pop_id`# 触发器与效果
| 0x000D | cash_reserves | 2 | Tests that the primary `dcon::pop_id` has an amount of cash reserves, told by ## 触发器与效果
| 0x000D | cash_reserves | 2 | Tests that the primary `dcon::pop_id` has an amount of cash reserves, told by #1 (`float`) |
| 0x000E | unemployment_nation | 2 |# 触发器与效果
| 0x000E | unemployment_nation | 2 | Unemployment of the primary `dcon::n# 触发器与效果
| 0x000E | unemployment_nation | 2 | Unemployment of the primary `dcon::nation_id` has #1 (`float`) |
| 0x000F | unemployment_state | 2 | Unemployment of the primary `# 触发器与效果
| 0x000F | unemployment_state | 2 | Unemployment of the primary `dcon::state_instance_id` has #1 (`float`) |
| 0x0010 | unemployment_province | 2 | Un# 触发器与效果
| 0x0010 | unemployment_province | 2 | Unemployment of the primary `dcon::province_id# 触发器与效果
| 0x0010 | unemployment_province | 2 | Unemployment of the primary `dcon::province_id` has #1 (`float`) |
| 0x0011 | unemployment_pop |# 触发器与效果
| 0x0011 | unemployment_pop | 2 | Unemployment of the primary `dcon# 触发器与效果
| 0x0011 | unemployment_pop | 2 | Unemployment of the primary `dcon::pop_id` has #1 (`float`) |
| 0x0012 | is# 触发器与效果
| 0x0012 | is_slave_nation | 0 | Is the primary# 触发器与效果
| 0x0012 | is_slave_nation | 0 | Is the primary `dcon::nation_id` a nation that allows slavery? |
| 0x0013 | is_slave_state | 0 |# 触发器与效果
| 0x0013 | is_slave_state | 0 | Is the primary `dcon::state_instance_id# 触发器与效果
| 0x0013 | is_slave_state | 0 | Is the primary `dcon::state_instance_id` a slave state? |
| 0x0014 | is_slave_province | 0 | Is the primary `dcon::province# 触发器与效果
| 0x0014 | is_slave_province | 0 | Is the primary `dcon::province_id` a province pertaining to a slave state?# 触发器与效果
| 0x0014 | is_slave_province | 0 | Is the primary `dcon::province_id` a province pertaining to a slave state? |
| 0x0015 | is# 触发器与效果
| 0x0015 | is_slave_pop | 0 | Is the primary `# 触发器与效果
| 0x0015 | is_slave_pop | 0 | Is the primary `dcon::pop_id` a slave? |
| 0x0016 | is_in# 触发器与效果
| 0x0016 | is_independant | 0 | Is the primary `# 触发器与效果
| 0x0016 | is_independant | 0 | Is the primary `dcon::nation_id` independent? (# 触发器与效果
| 0x0016 | is_independant | 0 | Is the primary `dcon::nation_id` independent? (No puppet overlord and not a substate) |
| 0x0017 | has_national_minority_province | 0 | **All** pops on `dcon::province# 触发器与效果
| 0x0017 | has_national_minority_province | 0 | **All** pops on `dcon::province_id` are of non-primary culture |
| 0x0018 | has_national_minor# 触发器与效果
| 0x0018 | has_national_minority_state | 0 | **All** pops# 触发器与效果
| 0x0018 | has_national_minority_state | 0 | **All** pops on `dcon::state_instance_id` are of non-primary culture |
| 0x# 触发器与效果
| 0x0019 | has_national_minority_nation# 触发器与效果
| 0x0019 | has_national_minority_nation | 0 | **All** pops on `# 触发器与效果
| 0x0019 | has_national_minority_nation | 0 | **All** pops on `dcon::nation_id` are of non# 触发器与效果
| 0x0019 | has_national_minority_nation | 0 | **All** pops on `dcon::nation_id` are of non-primary culture |
| 0x001A | government_nation | 1 | Government of# 触发器与效果
| 0x001A | government_nation | 1 | Government of primary `dcon::nation_id` are #1 (`dcon::goverment_type_id`) |
| 0x001B# 触发器与效果
| 0x001B | government_pop | 1 | Government of primary `dcon::pop_id` is #1# 触发器与效果
| 0x001B | government_pop | 1 | Government of primary `dcon::pop_id` is #1 (`dcon::goverment_type`) |
| 0x001C | capital |# 触发器与效果
| 0x001C | capital | 1 | Checks that primary `dcon::nation_id` capital is #1 (`d# 触发器与效果
| 0x001C | capital | 1 | Checks that primary `dcon::nation_id` capital is #1 (`dcon::province_id`) |
| 0x001D | tech_school | 1 |
| 0x001E | primary# 触发器与效果
| 0x001E | primary_culture | 1 |
| 0x001F | accepted_culture | 1# 触发器与效果
| 0x001F | accepted_culture | 1 |
| 0x0020 | culture# 触发器与效果
| 0x0020 | culture_pop | 1 |
| 0x0021 | culture_state | 1 |
| 0x0022 | culture_province# 触发器与效果
| 0x0022 | culture_province | 1 |
| 0x002# 触发器与效果
| 0x0023 | culture_nation | 1 |
| 0x0024 | culture_pop_reb | 0 |
| 0x0025 | culture_state_reb | 0# 触发器与效果
| 0x0025 | culture_state_reb | 0 |
| 0x0026 | culture_province_reb | 0 |
| 0x0027 | culture_nation_re# 触发器与效果
| 0x0027 | culture_nation_reb | 0 |
| 0x0028 | culture_from_nation | 0 |
| 0x0029 | culture# 触发器与效果
| 0x0029 | culture_this_nation | 0 |
| 0x002A | culture_this_state | 0 |
| 0x002B |# 触发器与效果
| 0x002B | culture_this_pop | 0 |
| 0x002C | culture_this_province |# 触发器与效果
| 0x002C | culture_this_province | 0 |
| 0x002D | culture_group_nation | 1 |
| 0x002E | culture_group_pop# 触发器与效果
| 0x002E | culture_group_pop | 1 |
| 0x002F | culture_group_reb_nation |# 触发器与效果
| 0x002F | culture_group_reb_nation | 0 |
| 0x0030 | culture_group_reb_pop | 0 |
| 0x0031 | culture_group_nation_from_nation | 0 |
| 0x0032 | culture_group_pop_from# 触发器与效果
| 0x0032 | culture_group_pop_from_nation | 0 |
| 0x0033 | culture_group_nation_this_nation | 0 |
| 0x0034 | culture_group_pop_this_nation | 0 |
| 0x0035# 触发器与效果
| 0x0035 | culture_group_nation_this_province |# 触发器与效果
| 0x0035 | culture_group_nation_this_province | 0 |
| 0x0036 | culture_group_pop_this_province | 0 |
| 0x0037 | culture_group# 触发器与效果
| 0x0037 | culture_group_nation_this_state | 0 |
| 0x0038 | culture_group_pop_this_state | 0 |
| 0x0039 | culture_group_nation_this_pop |# 触发器与效果
| 0x0039 | culture_group_nation_this_pop | 0 |
| 0x003A# 触发器与效果
| 0x003A | culture_group_pop_this_pop | 0 |
| 0x003B | religion |# 触发器与效果
| 0x003B | religion | 1 |
| 0x003C# 触发器与效果
| 0x003C | religion_reb | 0 |
| 0x003D | religion_from_nation | 0 |
| 0x003# 触发器与效果
| 0x003E | religion_this_nation | 0 |
| 0x003F | religion_this_state | 0 |
| 0x0040 | religion_this_province | 0 |
| 0x0041 | religion# 触发器与效果
| 0x0041 | religion_this_pop | 0 |
| 0x0042 | terrain_province | 1# 触发器与效果
| 0x0042 | terrain_province | 1 |
| 0x0043 | terrain# 触发器与效果
| 0x0043 | terrain_pop | 1 |
| 0x0044 | trade_goods | 1 |
| 0x0045 | is_secondary_power_pop | 0 |
| 0x0046 | is_secondary_power_nation |# 触发器与效果
| 0x0046 | is_secondary_power_nation | 0 |
| 0x0047 | has_faction_nation | 0 |
| 0x0048 | has_faction# 触发器与效果
| 0x0048 | has_faction_pop | 1 |
| 0x0049 | has_unclaimed_cores |# 触发器与效果
| 0x0049 | has_unclaimed_cores | 0 |
| 0x004A |# 触发器与效果
| 0x004A | is_cultural_union_bool | 0 |
| 0x004B | is_cultural# 触发器与效果
| 0x004B | is_cultural_union_this_self_pop | 0 |
| 0x004C | is_cultural_union_this_pop | 0 |
| 0x004D | is_cultural_union_this_state# 触发器与效果
| 0x004D | is_cultural_union_this_state | 0 |
| 0x004E | is_cultural_union_this_province | 0 |
| 0x004F# 触发器与效果
| 0x004F | is_cultural_union_this_nation | 0 |
| 0x0050 |# 触发器与效果
| 0x0050 | is_cultural_union_this_rebel | 0# 触发器与效果
| 0x0050 | is_cultural_union_this_rebel | 0 |
| 0x0051 | is# 触发器与效果
| 0x0051 | is_cultural_union_tag_nation | 1 |
| 0x0052 | is_cultural_union_tag_this_pop | 1 |
| 0x0053 | is_cultural_union_tag_this_state | 1 |
| 0x0054 | is_cultural_union_tag_this_province | 1 |
| 0x0055 | is_cultural_union# 触发器与效果
| 0x0055 | is_cultural_union_tag_this_nation | 1 |
| 0x0056 | can_build_factory_pop# 触发器与效果
| 0x0056 | can_build_factory_pop | 0 |
| 0x005# 触发器与效果
| 0x0057 | war_pop | 0 |
| 0x0058 | war_nation | 0 |
| 0x0059# 触发器与效果
| 0x0059 | war_exhaustion_nation | 2# 触发器与效果
| 0x0059 | war_exhaustion_nation | 2 | War exhaustion of primary `dcon::n# 触发器与效果
| 0x0059 | war_exhaustion_nation | 2 | War exhaustion of primary `dcon::nation_id` compared with #1 (`float`)# 触发器与效果
| 0x0059 | war_exhaustion_nation | 2 | War exhaustion of primary `dcon::nation_id` compared with #1 (`float`) |
| 0x005A | block# 触发器与效果
| 0x005A | blockade | 2 |
| 0x005B | owns | 1 | Primary `# 触发器与效果
| 0x005B | owns | 1 | Primary `dcon::nation_id` owns #1# 触发器与效果
| 0x005B | owns | 1 | Primary `dcon::nation_id` owns #1 (`dcon::province_id`) |
| 0x005C | controls | 1# 触发器与效果
| 0x005C | controls | 1 | Primary `dcon::nation_id` controls #1 (`dcon::province_id`) |
| 0x005D | is_core_integer | 1 |
| 0x005E | is_core_this_nation | 0 |
| 0x005F# 触发器与效果
| 0x005F | is_core_this_state | 0 |
| 0x0060 | is_core_this_province | 0 |
| 0x0061 | is_core_this_pop |# 触发器与效果
| 0x0061 | is_core_this_pop | 0 |
| 0x0062 |# 触发器与效果
| 0x0062 | is_core_from_nation | 0 |
| 0x0063 | is_core_re# 触发器与效果
| 0x0063 | is_core_reb | 0 |
| 0x0064 | is_core_tag | 1 |
| 0x0065 | num_of# 触发器与效果
| 0x0065 | num_of_revolts | 1 |
| 0x0066 | revolt_percentage |# 触发器与效果
| 0x0066 | revolt_percentage | 2 |
| 0x0067 | num_of_cities_int | 1 |
| 0x0068 | num_of_c# 触发器与效果
| 0x0068 | num_of_cities_from_nation | 0 |
| 0x0069 | num_of_cities_this_nation | 0 |
| 0x006A | num_of_cities_this# 触发器与效果
| 0x006A | num_of_cities_this_state | 0 |
| 0x006B | num_of_cities_this_province | 0 |
| 0x006# 触发器与效果
| 0x006C | num_of_cities_this_pop | 0 |
| 0x006D |# 触发器与效果
| 0x006D | num_of_ports | 1 |
| 0x006E | num_of_allies |# 触发器与效果
| 0x006E | num_of_allies | 1 |
| 0x006F | num_of_vassals | 1 |
| 0x0070 | owned_by# 触发器与效果
| 0x0070 | owned_by_tag | 1 |
| 0x0071 | owned_by_from_nation | 0 |
| 0x0072 | owned_by_this_nation | 0 |
| 0x0073 | owned_by_this_province | 0 |
| 0x0074 | owned_by_this_state |# 触发器与效果
| 0x0074 | owned_by_this_state | 0 |
| 0x0075 |# 触发器与效果
| 0x0075 | owned_by_this_pop | 0 |
| 0x0076 | exists_bool |# 触发器与效果
| 0x0076 | exists_bool | 0 |
| 0x0077 | exists_tag | 1 |
| 0x0078 | has_country_flag | 1# 触发器与效果
| 0x0078 | has_country_flag | 1 |
| 0x0079 | continent# 触发器与效果
| 0x0079 | continent_nation | 1 |
| 0x007A | continent_state | 1 |
| 0x007B | continent_province | 1 |
| 0x007C | continent_pop | 1 |
| 0x007D | continent_nation_this | 0 |
| 0x007E | continent_state_this | 0 |
| 0x007F | continent_prov# 触发器与效果
| 0x007F | continent_province_this | 0 |
| 0x0080 | continent_pop_this | 0 |
| 0x0081 | continent# 触发器与效果
| 0x0081 | continent_nation_from | 0 |
| 0x0082 | continent_state_from |# 触发器与效果
| 0x0082 | continent_state_from | 0 |
| 0x0083 |# 触发器与效果
| 0x0083 | continent_province_from | 0 |
| 0x0084 | continent_pop_from |# 触发器与效果
| 0x0084 | continent_pop_from | 0 |
| 0x0085# 触发器与效果
| 0x0085 | casus_belli_tag | 1 |
| 0x0086 | casus# 触发器与效果
| 0x0086 | casus_belli_from | 0 |
| 0x0087 | casus_belli_this_nation | 0 |
| 0x0088 | casus_belli_this_state# 触发器与效果
| 0x0088 | casus_belli_this_state | 0 |
| 0x0089 | casus_belli_this_province | 0 |
| 0x008A# 触发器与效果
| 0x008A | casus_belli_this_pop | 0# 触发器与效果
| 0x008A | casus_belli_this_pop | 0 |
| 0x008B | military# 触发器与效果
| 0x008B | military_access_tag | 1 |
| 0x008C | military_access_from | 0# 触发器与效果
| 0x008C | military_access_from | 0 |
| 0x008D | military_access_this_nation | 0 |
| 0x008E | military_access_this_state# 触发器与效果
| 0x008E | military_access_this_state | 0 |
| 0x008F | military_access_this_province | 0# 触发器与效果
| 0x008F | military_access_this_province | 0 |
| 0x0090 | military# 触发器与效果
| 0x0090 | military_access_this_pop | 0 |
| 0x0091 | prestige_value | 2# 触发器与效果
| 0x0091 | prestige_value | 2 |
| 0x0092 | prestige_from | 0 |
| 0x0093 | prestige_this_nation | 0# 触发器与效果
| 0x0093 | prestige_this_nation | 0 |
| 0x0094 | prestige_this_state | 0 |
| 0x0095 | prestige_this_province |# 触发器与效果
| 0x0095 | prestige_this_province | 0 |
| 0x0096 | prestige_this_pop | 0 |
| 0x0097 | badboy | 2# 触发器与效果
| 0x0097 | badboy | 2 |
| 0x0098 | has_building_state | 1 |
| 0x0099 | has_building_fort | 0 |
| 0x009# 触发器与效果
| 0x009A | has_building_railroad | 0 |
| 0x009B |# 触发器与效果
| 0x009B | has_building_naval_base | 0 |
| 0x009C | empty |# 触发器与效果
| 0x009C | empty | 0 |
| 0x009D | is_blockaded | 0 |
| 0x009E | has_country_modifier |# 触发器与效果
| 0x009E | has_country_modifier | 1 |
| 0x009F | has_province_modifier | 1 |
| 0x00A0 | region |# 触发器与效果
| 0x00A0 | region | 1 |
| 0x00A# 触发器与效果
| 0x00A1 | tag_tag | 1 |
| 0x00A2 | tag_this_n# 触发器与效果
| 0x00A2 | tag_this_nation | 0 |
| 0x00A3 | tag_this_province |# 触发器与效果
| 0x00A3 | tag_this_province | 0 |
| 0x00A4 | tag_from_nation | 0 |
| 0x00A5 | tag_from_province | 0 |
| 0x00A6 | neighbour_tag | 1# 触发器与效果
| 0x00A6 | neighbour_tag | 1 |
| 0x00A7 | neighbour_this | 0 |
| 0x00A8 | neighbour_from | 0# 触发器与效果
| 0x00A8 | neighbour_from | 0 |
| 0x00A9 | units_in_province_value | 1 |
| 0x00AA | units_in_prov# 触发器与效果
| 0x00AA | units_in_province_from | 0 |
| 0x00AB | units_in_province_this_n# 触发器与效果
| 0x00AB | units_in_province_this_nation | 0 |
| 0x00AC | units_in_province_this_province | 0 |
| 0x00# 触发器与效果
| 0x00AD | units_in_province_this_state | 0 |
| 0x00AE |# 触发器与效果
| 0x00AE | units_in_province_this_pop | 0 |
| 0x00AF | war_with# 触发器与效果
| 0x00AF | war_with_tag | 1 |
| 0x00B0 | war_with_from | 0# 触发器与效果
| 0x00B0 | war_with_from | 0 |
| 0x00B1 | war_with_this_nation | 0 |
| 0x00B2 | war_with# 触发器与效果
| 0x00B2 | war_with_this_province | 0 |
| 0x00B3 | war_with_this_state# 触发器与效果
| 0x00B3 | war_with_this_state | 0 |
| 0x00B4 | war_with_this_pop | 0# 触发器与效果
| 0x00B4 | war_with_this_pop | 0 |
| 0x00B5 | unit_in_battle | 0 |
| 0x00B6 | total_amount_of# 触发器与效果
| 0x00B6 | total_amount_of_divisions | 1 |
| 0x00B7 | money | 2 |
| 0x00B8 | lost# 触发器与效果
| 0x00B8 | lost_national | 2 |
| 0x00B9 | is_vassal |# 触发器与效果
| 0x00B9 | is_vassal | 0 |
| 0x00BA | ruling_party_ideology_pop | 1 |
| 0x00BB | ruling# 触发器与效果
| 0x00BB | ruling_party_ideology_nation | 1 |
| 0x00BC | ruling_party# 触发器与效果
| 0x00BC | ruling_party | 1 |
| 0x00BD | is_ideology_enabled | 1# 触发器与效果
| 0x00BD | is_ideology_enabled | 1 |
| 0x00BE | political# 触发器与效果
| 0x00BE | political_reform_want_nation | 2 |
| 0x00BF | political_re# 触发器与效果
| 0x00BF | political_reform_want_pop | 2 |
| 0x00C0 | social_reform# 触发器与效果
| 0x00C0 | social_reform_want_nation | 2 |
| 0x00C1 | social_reform_want_pop | 2 |
| 0x00C2 | total_amount_of_s# 触发器与效果
| 0x00C2 | total_amount_of_ships | 1 |
| 0x00C3 | plurality | 2 |
| 0x00C4 | corruption |# 触发器与效果
| 0x00C4 | corruption | 2 |
| 0x00C5 | is_state_religion_pop |# 触发器与效果
| 0x00C5 | is_state_religion_pop | 0 |
| 0x00C6# 触发器与效果
| 0x00C6 | is_state_religion_province |# 触发器与效果
| 0x00C6 | is_state_religion_province | 0 |
| 0x00C7# 触发器与效果
| 0x00C7 | is_state_religion_state | 0 |
| 0x00C8 |# 触发器与效果
| 0x00C8 | is_primary_culture_pop | 0 |
| 0x00C9 | is_primary_culture_province | 0 |
| 0x00CA | is_primary_culture_state | 0 |
| 0x00CB | is_primary_culture_nation_this_pop | 0 |
| 0x00CC | is_primary_culture_nation_this# 触发器与效果
| 0x00CC | is_primary_culture_nation_this_nation | 0 |
| 0x00CD | is_primary_culture_nation# 触发器与效果
| 0x00CD | is_primary_culture_nation_this_state | 0 |
| 0x00CE | is_primary_culture_nation# 触发器与效果
| 0x00CE | is_primary_culture_nation_this_province | 0 |
| 0x00CF | is_primary_culture_state_this_pop | 0 |
| 0x00D0 | is_primary_culture_state# 触发器与效果
| 0x00D0 | is_primary_culture_state_this_nation | 0 |
| 0x00D1 | is_primary_culture# 触发器与效果
| 0x00D1 | is_primary_culture_state_this_state | 0 |
| 0x00D2 | is_primary_culture# 触发器与效果
| 0x00D2 | is_primary_culture_state_this_province | 0 |
| 0x00D3 | is_primary_c# 触发器与效果
| 0x00D3 | is_primary_culture_province_this_pop | 0 |
| 0x00D4 | is_primary# 触发器与效果
| 0x00D4 | is_primary_culture_province_this_nation | 0# 触发器与效果
| 0x00D4 | is_primary_culture_province_this_nation | 0 |
| 0x00D5 | is_primary_culture_province_this_state |# 触发器与效果
| 0x00D5 | is_primary_culture_province_this_state | 0 |
| 0x00D6# 触发器与效果
| 0x00D6 | is_primary_culture_province_this_province# 触发器与效果
| 0x00D6 | is_primary_culture_province_this_province | 0 |
| 0x00D7 | is_primary_culture_pop_this_pop# 触发器与效果
| 0x00D7 | is_primary_culture_pop_this_pop | 0 |
| 0x00D8 | is_primary_culture_pop_this_n# 触发器与效果
| 0x00D8 | is_primary_culture_pop_this_nation | 0 |
| 0x00D9 | is_primary_culture_pop_this# 触发器与效果
| 0x00D9 | is_primary_culture_pop_this_state | 0 |
| 0x00DA | is_primary_culture_pop_this_province | 0 |
| 0x00DB | is_accepted_culture_pop |# 触发器与效果
| 0x00DB | is_accepted_culture_pop | 0 |
| 0x00DC# 触发器与效果
| 0x00DC | is_accepted_culture_province |# 触发器与效果
| 0x00DC | is_accepted_culture_province | 0 |
| 0x00DD | is_accepted_culture_state | 0 |
| 0x00DE | is_co# 触发器与效果
| 0x00DE | is_coastal | 0 |
| 0x00DF | in_sphere_tag | 1 |
| 0x00E0 | in# 触发器与效果
| 0x00E0 | in_sphere_from | 0 |
| 0x00E1 | in_sphere_this_nation# 触发器与效果
| 0x00E1 | in_sphere_this_nation | 0 |
| 0x00E2 | in_sphere_this_province |# 触发器与效果
| 0x00E2 | in_sphere_this_province | 0 |
| 0x00E3 | in_sphere_this_state | 0 |
| 0x00E4 | in_sphere# 触发器与效果
| 0x00E4 | in_sphere_this_pop | 0 |
| 0x00E5 | produces_nation |# 触发器与效果
| 0x00E5 | produces_nation | 1 |
| 0x00E6# 触发器与效果
| 0x00E6 | produces_state | 1 |
| 0x00E7 | produces_province |# 触发器与效果
| 0x00E7 | produces_province | 1 |
| 0x00E# 触发器与效果
| 0x00E8 | produces_pop | 1 |
| 0x00E9 | average_milit# 触发器与效果
| 0x00E9 | average_militancy_nation | 2 |
| 0x00EA | average_militancy_state | 2 |
| 0x00EB | average_militancy_province |# 触发器与效果
| 0x00EB | average_militancy_province | 2 |
| 0x00EC |# 触发器与效果
| 0x00EC | average_consciousness_nation | 2 |
| 0x00ED | average_con# 触发器与效果
| 0x00ED | average_consciousness_state | 2 |
| 0x00EE | average_consciousness_prov# 触发器与效果
| 0x00EE | average_consciousness_province | 2 |
| 0x00EF | is_next_reform_nation |# 触发器与效果
| 0x00EF | is_next_reform_nation | 1 |
| 0x00F# 触发器与效果
| 0x00F0 | is_next_reform_pop | 1# 触发器与效果
| 0x00F0 | is_next_reform_pop | 1 |
| 0x00F1 |# 触发器与效果
| 0x00F1 | rebel_power_fraction | 2 |
| 0x00F2 | recruited_percentage_nation# 触发器与效果
| 0x00F2 | recruited_percentage_nation | 2 |
| 0x00F3 | recruited_percentage_pop | 2 |
| 0x00F4 | has# 触发器与效果
| 0x00F4 | has_culture_core | 0 |
| 0x00F5 | nationalism | 1# 触发器与效果
| 0x00F5 | nationalism | 1 |
| 0x00F6 |# 触发器与效果
| 0x00F6 | is_overseas | 0 |
| 0x00F7 | controlled_by_re# 触发器与效果
| 0x00F7 | controlled_by_rebels | 0 |
| 0x00F8 | controlled_by_tag |# 触发器与效果
| 0x00F8 | controlled_by_tag | 1 |
| 0x00F9# 触发器与效果
| 0x00F9 | controlled_by_from | 0 |
| 0x00FA | controlled_by_this_n# 触发器与效果
| 0x00FA | controlled_by_this_nation | 0 |
| 0x00FB | controlled_by_this_province |# 触发器与效果
| 0x00FB | controlled_by_this_province | 0 |
| 0x00FC | controlled_by_this_state | 0 |
| 0x00FD | controlled_by_this_pop# 触发器与效果
| 0x00FD | controlled_by_this_pop | 0 |
| 0x00FE | controlled_by_owner | 0 |
| 0x00FF | controlled_by_re# 触发器与效果
| 0x00FF | controlled_by_reb | 0 |
| 0x0100 | is_canal_enabled | 1# 触发器与效果
| 0x0100 | is_canal_enabled | 1 |
| 0x0101 | is# 触发器与效果
| 0x0101 | is_state_capital | 0 |
| 0x0102 | truce_with_tag |# 触发器与效果
| 0x0102 | truce_with_tag | 1 |
| 0x0103# 触发器与效果
| 0x0103 | truce_with_from | 0 |
| 0x0104 | truce_with# 触发器与效果
| 0x0104 | truce_with_this_nation | 0 |
| 0x0105 | truce_with_this_province | 0 |
| 0x0106 | truce_with_this_state |# 触发器与效果
| 0x0106 | truce_with_this_state | 0 |
| 0x0107 |# 触发器与效果
| 0x0107 | truce_with_this_pop | 0 |
| 0x0108 | total_pops# 触发器与效果
| 0x0108 | total_pops_nation | 2 |
| 0x0109 | total_pops_state |# 触发器与效果
| 0x0109 | total_pops_state | 2 |
| 0x010A | total_pops_province | 2 |
| 0x010B | total_pops# 触发器与效果
| 0x010B | total_pops_pop | 2 |
| 0x010C | has_pop_type_nation |# 触发器与效果
| 0x010C | has_pop_type_nation | 1 |
| 0x010D |# 触发器与效果
| 0x010D | has_pop_type_state | 1 |
| 0x010E | has_pop_type_prov# 触发器与效果
| 0x010E | has_pop_type_province | 1 |
| 0x010F | has_pop_type_pop | 1# 触发器与效果
| 0x010F | has_pop_type_pop | 1 |
| 0x0110 | has# 触发器与效果
| 0x0110 | has_empty_adjacent_province | 0 |
| 0x0111 | has_leader | 1 |
| 0x0112# 触发器与效果
| 0x0112 | ai | 0 |
| 0x0113 | can_create_vassals |# 触发器与效果
| 0x0113 | can_create_vassals | 0 |
| 0x0114# 触发器与效果
| 0x0114 | is_possible_vassal | 1 |
| 0x0115 | province_id# 触发器与效果
| 0x0115 | province_id | 1 |
| 0x0116 | vassal_of_tag | 1# 触发器与效果
| 0x0116 | vassal_of_tag | 1 |
| 0x0117 | v# 触发器与效果
| 0x0117 | vassal_of_from | 0 |
| 0x0118 | vassal_of# 触发器与效果
| 0x0118 | vassal_of_this_nation | 0 |
| 0x0119 | vassal_of_this_province | 0 |
| 0x011A | vassal_of_this_state# 触发器与效果
| 0x011A | vassal_of_this_state | 0 |
| 0x011# 触发器与效果
| 0x011B | vassal_of_this_pop |# 触发器与效果
| 0x011B | vassal_of_this_pop | 0 |
| 0x011C | alliance_with_tag | 1 |
| 0x011D | alliance_with_from |# 触发器与效果
| 0x011D | alliance_with_from | 0 |
| 0x011E |# 触发器与效果
| 0x011E | alliance_with_this_nation | 0 |
| 0x011F | alliance_with_this# 触发器与效果
| 0x011F | alliance_with_this_province | 0 |
| 0x0120 | alliance_with_this_state |# 触发器与效果
| 0x0120 | alliance_with_this_state | 0 |
| 0x0121 |# 触发器与效果
| 0x0121 | alliance_with_this_pop | 0 |
| 0x0122 | has_recently_l# 触发器与效果
| 0x0122 | has_recently_lost_war | 0 |
| 0x0123 | is_mobilised | 0 |
| 0x0124# 触发器与效果
| 0x0124 | mobilisation_size | 2 |
| 0x0125 | crime_higher_than# 触发器与效果
| 0x0125 | crime_higher_than_education_nation | 0 |
| 0x0126 | crime_higher_than# 触发器与效果
| 0x0126 | crime_higher_than_education_state | 0 |
| 0x0127 | crime_higher_than_education_province | 0 |
| 0x0128 | crime_higher_than_ed# 触发器与效果
| 0x0128 | crime_higher_than_education_pop | 0 |
| 0x0129 | agree_with_ruling_party | 2 |
| 0x012A# 触发器与效果
| 0x012A | is_colonial_state | 0 |
| 0x012B | is_colonial# 触发器与效果
| 0x012B | is_colonial_province | 0 |
| 0x012C | has_factories | 0# 触发器与效果
| 0x012C | has_factories | 0 |
| 0x012D | in# 触发器与效果
| 0x012D | in_default_tag | 1 |
| 0x012E | in_default_from | 0# 触发器与效果
| 0x012E | in_default_from | 0 |
| 0x012F | in# 触发器与效果
| 0x012F | in_default_this_nation | 0 |
| 0x0130 | in_default_this_prov# 触发器与效果
| 0x0130 | in_default_this_province | 0 |
| 0x0131 | in_default_this_state | 0# 触发器与效果
| 0x0131 | in_default_this_state | 0 |
| 0x0132 | in# 触发器与效果
| 0x0132 | in_default_this_pop | 0 |
| 0x0133 | total_num_of_ports |# 触发器与效果
| 0x0133 | total_num_of_ports | 1 |
| 0x0134# 触发器与效果
| 0x0134 | always | 0 |
| 0x0135 | election | 0 |
| 0x0136 | has_global_flag# 触发器与效果
| 0x0136 | has_global_flag | 1 |
| 0x013# 触发器与效果
| 0x0137 | is_capital | 0 |
| 0x0138 | nationalvalue_n# 触发器与效果
| 0x0138 | nationalvalue_nation | 1 |
| 0x0139 | industrial_score_value | 1 |
| 0x013A | industrial_score# 触发器与效果
| 0x013A | industrial_score_from_nation | 0 |
| 0x013B | industrial_score_this_nation# 触发器与效果
| 0x013B | industrial_score_this_nation | 0 |
| 0x013C | industrial_score_this_pop | 0 |
| 0x013D | industrial_score# 触发器与效果
| 0x013D | industrial_score_this_state | 0 |
| 0x013E | industrial_score_this_province |# 触发器与效果
| 0x013E | industrial_score_this_province | 0 |
| 0x013F# 触发器与效果
| 0x013F | military_score_value | 1 |
| 0x0140 | military_score_from_n# 触发器与效果
| 0x0140 | military_score_from_nation | 0 |
| 0x0141 | military_score_this_nation |# 触发器与效果
| 0x0141 | military_score_this_nation | 0 |
| 0x0142 | military_score_this_pop | 0 |
| 0x0143 | military_score_this_state# 触发器与效果
| 0x0143 | military_score_this_state | 0 |
| 0x014# 触发器与效果
| 0x0144 | military_score_this_province | 0# 触发器与效果
| 0x0144 | military_score_this_province | 0 |
| 0x0145 | civil# 触发器与效果
| 0x0145 | civilized_nation | 0 |
| 0x0146 | civilized_pop |# 触发器与效果
| 0x0146 | civilized_pop | 0 |
| 0x0147 |# 触发器与效果
| 0x0147 | civilized_province | 0 |
| 0x0148 | national_provinces# 触发器与效果
| 0x0148 | national_provinces_occupied | 2 |
| 0x0149 | is_greater_power_nation | 0 |
| 0x014A | is_greater_power_pop | 0# 触发器与效果
| 0x014A | is_greater_power_pop | 0 |
| 0x014B | rich# 触发器与效果
| 0x014B | rich_tax | 1 |
| 0x014C | middle_tax | 1 |
| 0x014D | poor_tax |# 触发器与效果
| 0x014D | poor_tax | 1 |
| 0x014E | social_spending_nation | 1 |
| 0x014F | social_sp# 触发器与效果
| 0x014F | social_spending_pop | 1 |
| 0x0150 | social_spending_province |# 触发器与效果
| 0x0150 | social_spending_province | 1 |
| 0x0151# 触发器与效果
| 0x0151 | colonial_nation | 0 |
| 0x0152 | pop_majority_re# 触发器与效果
| 0x0152 | pop_majority_religion_nation | 1 |
| 0x0153 | pop_majority_re# 触发器与效果
| 0x0153 | pop_majority_religion_state | 1 |
| 0x0154 | pop_majority_religion_province | 1 |
| 0x0155 | pop_majority_culture# 触发器与效果
| 0x0155 | pop_majority_culture_nation | 1 |
| 0x0156 | pop_majority_culture_state | 1 |
| 0x015# 触发器与效果
| 0x0157 | pop_majority_culture_province |# 触发器与效果
| 0x0157 | pop_majority_culture_province | 1 |
| 0x0158# 触发器与效果
| 0x0158 | pop_majority_issue_nation | 2# 触发器与效果
| 0x0158 | pop_majority_issue_nation | 2 |
| 0x0159 | pop# 触发器与效果
| 0x0159 | pop_majority_issue_state | 2 |
| 0x015A | pop_majority_issue# 触发器与效果
| 0x015A | pop_majority_issue_province | 2 |
| 0x015B | pop_majority_issue_pop |# 触发器与效果
| 0x015B | pop_majority_issue_pop | 2 |
| 0x015C# 触发器与效果
| 0x015C | pop_majority_ideology_nation | 1 |
| 0x015D# 触发器与效果
| 0x015D | pop_majority_ideology_state |# 触发器与效果
| 0x015D | pop_majority_ideology_state | 1 |
| 0x015E |# 触发器与效果
| 0x015E | pop_majority_ideology_province |# 触发器与效果
| 0x015E | pop_majority_ideology_province | 1 |
| 0x015F | pop_majority_ideology_pop | 1# 触发器与效果
| 0x015F | pop_majority_ideology_pop | 1 |
| 0x0160 | poor# 触发器与效果
| 0x0160 | poor_strata_militancy_nation | 2# 触发器与效果
| 0x0160 | poor_strata_militancy_nation | 2 |
| 0x0161 | poor# 触发器与效果
| 0x0161 | poor_strata_militancy_state | 2 |
| 0x0162 | poor_str# 触发器与效果
| 0x0162 | poor_strata_militancy_province | 2 |
| 0x0163 | poor_str# 触发器与效果
| 0x0163 | poor_strata_militancy_pop | 2 |
| 0x0164 | middle_strata_militancy_nation | 2 |
| 0x0165 | middle_strata# 触发器与效果
| 0x0165 | middle_strata_militancy_state | 2 |
| 0x0166 | middle_strata_militancy_province | 2 |
| 0x0167 | middle_strata_m# 触发器与效果
| 0x0167 | middle_strata_militancy_pop | 2 |
| 0x0168 | rich_strata_militancy_nation | 2 |
| 0x0169 | rich_strata_milit# 触发器与效果
| 0x0169 | rich_strata_militancy_state | 2 |
| 0x016A | rich_strata_militancy_province | 2 |
| 0x016B | rich_strata_militancy# 触发器与效果
| 0x016B | rich_strata_militancy_pop | 2 |
| 0x016C | rich_tax_above_poor |# 触发器与效果
| 0x016C | rich_tax_above_poor | 0 |
| 0x016D | culture_has_union_tag_pop | 0 |
| 0x016E | culture_has_union# 触发器与效果
| 0x016E | culture_has_union_tag_nation | 0 |
| 0x016F | this_culture_union_tag# 触发器与效果
| 0x016F | this_culture_union_tag | 1 |
| 0x017# 触发器与效果
| 0x0170 | this_culture_union_from | 0# 触发器与效果
| 0x0170 | this_culture_union_from | 0 |
| 0x0171 | this# 触发器与效果
| 0x0171 | this_culture_union_this_nation | 0 |
| 0x0172 | this_c# 触发器与效果
| 0x0172 | this_culture_union_this_province | 0 |
| 0x0173 | this_culture# 触发器与效果
| 0x0173 | this_culture_union_this_state | 0 |
| 0x0174 | this_culture_union_this_pop | 0 |
| 0x0175 | this_culture_union_this_union_n# 触发器与效果
| 0x0175 | this_culture_union_this_union_nation | 0 |
| 0x0176 | this_culture_union_this_union_prov# 触发器与效果
| 0x0176 | this_culture_union_this_union_province | 0 |
| 0x0177 | this_culture_union_this_union_state# 触发器与效果
| 0x0177 | this_culture_union_this_union_state | 0 |
| 0x0178 | this_culture_union_this_union_pop | 0 |
| 0x0179# 触发器与效果
| 0x0179 | minorities_nation | 0 | Primary `# 触发器与效果
| 0x0179 | minorities_nation | 0 | Primary `dcon::nation_id`
| 0x017A | minorities_state | 0# 触发器与效果
| 0x017A | minorities_state | 0 |
| 0x017B | minorities# 触发器与效果
| 0x017B | minorities_province | 0 |
| 0x017C | revanchism_nation | 2 |
| 0x017D# 触发器与效果
| 0x017D | revanchism_pop | 2 |
| 0x017E | has_crime# 触发器与效果
| 0x017E | has_crime | 1 |
| 0x017F | num_of_substates | 1 |
| 0x0180 | num_of# 触发器与效果
| 0x0180 | num_of_vassals_no_substates | 1 |
| 0x0181 | brigades# 触发器与效果
| 0x0181 | brigades_compare_this | 2 |
| 0x0182 | brigades_compare_from |# 触发器与效果
| 0x0182 | brigades_compare_from | 2 |
| 0x0183 | constructing_cb_tag | 1 |
| 0x0184 | constructing_cb_from |# 触发器与效果
| 0x0184 | constructing_cb_from | 0 |
| 0x0185 |# 触发器与效果
| 0x0185 | constructing_cb_this_nation | 0 |
| 0x0186 | constructing_cb_this# 触发器与效果
| 0x0186 | constructing_cb_this_province | 0 |
| 0x0187 | constructing_cb_this_state |# 触发器与效果
| 0x0187 | constructing_cb_this_state | 0 |
| 0x0188 |# 触发器与效果
| 0x0188 | constructing_cb_this_pop | 0 |
| 0x0189 | constructing_cb_discovered# 触发器与效果
| 0x0189 | constructing_cb_discovered | 0 |
| 0x018# 触发器与效果
| 0x018A | constructing_cb_progress | 2 |
| 0x018B | civilization_progress |# 触发器与效果
| 0x018B | civilization_progress | 2 |
| 0x018C | constructing_cb_type | 1 |
| 0x018D | is_our_v# 触发器与效果
| 0x018D | is_our_vassal_tag | 1 |
| 0x018E | is_our_vass# 触发器与效果
| 0x018E | is_our_vassal_from | 0 |
| 0x018F | is_our_vassal_this_nation | 0 |
| 0x0190 | is_our_vass# 触发器与效果
| 0x0190 | is_our_vassal_this_province | 0 |
| 0x0191 | is_our_v# 触发器与效果
| 0x0191 | is_our_vassal_this_state | 0 |
| 0x0192 | is_our_v# 触发器与效果
| 0x0192 | is_our_vassal_this_pop | 0 |
| 0x0193 | substate_of_tag# 触发器与效果
| 0x0193 | substate_of_tag | 1 |
| 0x019# 触发器与效果
| 0x0194 | substate_of_from | 0 |
| 0x0195 | substate# 触发器与效果
| 0x0195 | substate_of_this_nation | 0 |
| 0x0196 | substate_of_this_province | 0 |
| 0x0197 | substate_of_this_state |# 触发器与效果
| 0x0197 | substate_of_this_state | 0 |
| 0x0198# 触发器与效果
| 0x0198 | substate_of_this_pop | 0 |
| 0x0199 | is_sub# 触发器与效果
| 0x0199 | is_substate | 0 |
| 0x019A | great_wars_enabled | 0# 触发器与效果
| 0x019A | great_wars_enabled | 0 |
| 0x019B | can# 触发器与效果
| 0x019B | can_nationalize | 0 |
| 0x019C | part_of_sphere |# 触发器与效果
| 0x019C | part_of_sphere | 0 |
| 0x019D |# 触发器与效果
| 0x019D | is_sphere_leader_of_tag | 1 |
| 0x019E | is_sphere_leader# 触发器与效果
| 0x019E | is_sphere_leader_of_from | 0 |
| 0x019F | is_sphere_leader_of_this_n# 触发器与效果
| 0x019F | is_sphere_leader_of_this_nation | 0 |
| 0x01A0 | is_sphere_leader_of_this_prov# 触发器与效果
| 0x01A0 | is_sphere_leader_of_this_province | 0 |
| 0x01A1 | is_sphere_leader_of_this_state# 触发器与效果
| 0x01A1 | is_sphere_leader_of_this_state | 0 |
| 0x01# 触发器与效果
| 0x01A2 | is_sphere_leader_of_this_pop | 0 |
| 0x01A# 触发器与效果
| 0x01A3 | number_of_states | 1 |
| 0x01A4 | war_score# 触发器与效果
| 0x01A4 | war_score | 2 |
| 0x01A5 | is_releasable_vass# 触发器与效果
| 0x01A5 | is_releasable_vassal_from | 0 |
| 0x01A6 | is_releasable# 触发器与效果
| 0x01A6 | is_releasable_vassal_other | 0 |
| 0x01A7 | has_recent_im# 触发器与效果
| 0x01A7 | has_recent_imigration | 1 |
| 0x01A8 | province_control_days | 1# 触发器与效果
| 0x01A8 | province_control_days | 1 |
| 0x01A9 | is_disarmed | 0 |
| 0x01AA | big_producer |# 触发器与效果
| 0x01AA | big_producer | 1 |
| 0x01AB |# 触发器与效果
| 0x01AB | someone_can_form_union_tag_from | 0 |
| 0x01AC | someone_can# 触发器与效果
| 0x01AC | someone_can_form_union_tag_other | 0 |
| 0x01AD | social_movement_st# 触发器与效果
| 0x01AD | social_movement_strength | 2 |
| 0x01AE | political_movement_strength |# 触发器与效果
| 0x01AE | political_movement_strength | 2 |
| 0x01AF | can_build_factory_in_capital_state | 1# 触发器与效果
| 0x01AF | can_build_factory_in_capital_state | 1 |
| 0x01B0 |# 触发器与效果
| 0x01B0 | social_movement | 0 |
| 0x01B1 | political_movement |# 触发器与效果
| 0x01B1 | political_movement | 0 |
| 0x01B# 触发器与效果
| 0x01B2 | has_cultural_sphere | 0 |
| 0x01B3 | world# 触发器与效果
| 0x01B3 | world_wars_enabled | 0 |
| 0x01B4 | has_pop_culture# 触发器与效果
| 0x01B4 | has_pop_culture_pop_this_pop | 0 |
| 0x01B5 | has_pop_culture# 触发器与效果
| 0x01B5 | has_pop_culture_state_this_pop | 0 |
| 0x01B6 | has_pop_culture# 触发器与效果
| 0x01B6 | has_pop_culture_province_this_pop | 0 |
| 0x01B7 | has_pop_c# 触发器与效果
| 0x01B7 | has_pop_culture_nation_this_pop | 0 |
| 0x01B8 | has_pop# 触发器与效果
| 0x01B8 | has_pop_culture_pop | 1 |
| 0x01B9 | has_pop_culture# 触发器与效果
| 0x01B9 | has_pop_culture_state | 1 |
| 0x01BA | has_pop_culture_province |# 触发器与效果
| 0x01BA | has_pop_culture_province | 1 |
| 0x01BB# 触发器与效果
| 0x01BB | has_pop_culture_nation | 1# 触发器与效果
| 0x01BB | has_pop_culture_nation | 1 |
| 0x01BC | has# 触发器与效果
| 0x01BC | has_pop_religion_pop_this_pop | 0# 触发器与效果
| 0x01BC | has_pop_religion_pop_this_pop | 0 |
| 0x01BD | has# 触发器与效果
| 0x01BD | has_pop_religion_state_this_pop | 0# 触发器与效果
| 0x01BD | has_pop_religion_state_this_pop | 0 |
| 0x01BE | has# 触发器与效果
| 0x01BE | has_pop_religion_province_this_pop |# 触发器与效果
| 0x01BE | has_pop_religion_province_this_pop | 0 |
| 0x01BF | has_pop_religion_nation_this_pop |# 触发器与效果
| 0x01BF | has_pop_religion_nation_this_pop | 0 |
| 0x01C# 触发器与效果
| 0x01C0 | has_pop_religion_pop |# 触发器与效果
| 0x01C0 | has_pop_religion_pop | 1 |
| 0x01C1# 触发器与效果
| 0x01C1 | has_pop_religion_state | 1# 触发器与效果
| 0x01C1 | has_pop_religion_state | 1 |
| 0x01C2 |# 触发器与效果
| 0x01C2 | has_pop_religion_province | 1# 触发器与效果
| 0x01C2 | has_pop_religion_province | 1 |
| 0x01C3 | has_pop_religion_nation | 1# 触发器与效果
| 0x01C3 | has_pop_religion_nation | 1 |
| 0x01C4 |# 触发器与效果
| 0x01C4 | life_needs | 2 | Life needs of# 触发器与效果
| 0x01C4 | life_needs | 2 | Life needs of primary `dcon::pop_id` compared against# 触发器与效果
| 0x01C4 | life_needs | 2 | Life needs of primary `dcon::pop_id` compared against #1 (`float`) |
| 0x01C5 | everyday_needs |# 触发器与效果
| 0x01C5 | everyday_needs | 2 | Everyday needs of primary `dcon::# 触发器与效果
| 0x01C5 | everyday_needs | 2 | Everyday needs of primary `dcon::pop_id` compared against #1 (`float`)# 触发器与效果
| 0x01C5 | everyday_needs | 2 | Everyday needs of primary `dcon::pop_id` compared against #1 (`float`) |
| 0x01C6 | luxury_needs | 2 | Luxury needs of# 触发器与效果
| 0x01C6 | luxury_needs | 2 | Luxury needs of primary `dcon::pop_id` compared against# 触发器与效果
| 0x01C6 | luxury_needs | 2 | Luxury needs of primary `dcon::pop_id` compared against #1 (`float`) |
| 0x01C7 | consciousness_pop | 2# 触发器与效果
| 0x01C7 | consciousness_pop | 2 | Consciousness of primary `dcon::pop# 触发器与效果
| 0x01C7 | consciousness_pop | 2 | Consciousness of primary `dcon::pop_id` compared against #1 (`float`) |
| 0x01C8 | consciousness# 触发器与效果
| 0x01C8 | consciousness_province | 2 | Average consciousness of primary# 触发器与效果
| 0x01C8 | consciousness_province | 2 | Average consciousness of primary `dcon::province_id` compared against ## 触发器与效果
| 0x01C8 | consciousness_province | 2 | Average consciousness of primary `dcon::province_id` compared against #1 (`float`) |
| 0x01C9 | consciousness_state | 2 |# 触发器与效果
| 0x01C9 | consciousness_state | 2 | Average consciousness of primary `dcon::state_instance# 触发器与效果
| 0x01C9 | consciousness_state | 2 | Average consciousness of primary `dcon::state_instance_id` compared against #1 (`float`) |
| 0x01CA | consciousness_n# 触发器与效果
| 0x01CA | consciousness_nation | 2 | Average consciousness of primary `# 触发器与效果
| 0x01CA | consciousness_nation | 2 | Average consciousness of primary `dcon::nation_id` compared against ## 触发器与效果
| 0x01CA | consciousness_nation | 2 | Average consciousness of primary `dcon::nation_id` compared against #1 (`float`) |
| 0x01CB | literacy_pop | 2 |
| 0x01CC | literacy_province | 2 |
| 0x01CD | literacy_state | 2 |
| 0x01CE | literacy_nation |# 触发器与效果
| 0x01CE | literacy_nation | 2 |
| 0x01CF# 触发器与效果
| 0x01CF | militancy_pop | 2 | Militancy# 触发器与效果
| 0x01CF | militancy_pop | 2 | Militancy of primary `dcon::pop_id` compared# 触发器与效果
| 0x01CF | militancy_pop | 2 | Militancy of primary `dcon::pop_id` compared against #1 (`float`) |
| 0x01D0 | militancy_province# 触发器与效果
| 0x01D0 | militancy_province | 2 | Average militancy of primary `# 触发器与效果
| 0x01D0 | militancy_province | 2 | Average militancy of primary `dcon::province_id` compared against #1# 触发器与效果
| 0x01D0 | militancy_province | 2 | Average militancy of primary `dcon::province_id` compared against #1 (`float`) |
| 0x01D1 | militancy_state | 2 |# 触发器与效果
| 0x01D1 | militancy_state | 2 | Average militancy of primary `dcon::state# 触发器与效果
| 0x01D1 | militancy_state | 2 | Average militancy of primary `dcon::state_instance_id` compared against #1 (`float`) |
| 0x01D2 |# 触发器与效果
| 0x01D2 | militancy_nation | 2 | Average milit# 触发器与效果
| 0x01D2 | militancy_nation | 2 | Average militancy of primary `dcon::nation_id# 触发器与效果
| 0x01D2 | militancy_nation | 2 | Average militancy of primary `dcon::nation_id` compared against #1 (`float`) |
| 0x01D3 | military_sp# 触发器与效果
| 0x01D3 | military_spending_pop | 1 |
| 0x01D4 | military_spending_province# 触发器与效果
| 0x01D4 | military_spending_province | 1 |
| 0x01D5 | military_spending_state | 1# 触发器与效果
| 0x01D5 | military_spending_state | 1 |
| 0x01D6 | military_spending_nation | 1 |
| 0x01D7 | administration_sp# 触发器与效果
| 0x01D7 | administration_spending_pop | 1 |
| 0x01D8 | administration_spending_province# 触发器与效果
| 0x01D8 | administration_spending_province | 1 |
| 0x01D9 | administration_spending_state | 1# 触发器与效果
| 0x01D9 | administration_spending_state | 1 |
| 0x01DA | administration# 触发器与效果
| 0x01DA | administration_spending_nation | 1 |
| 0x01DB | education_spending_pop# 触发器与效果
| 0x01DB | education_spending_pop | 1 |
| 0x01DC | education_spending_province | 1# 触发器与效果
| 0x01DC | education_spending_province | 1 |
| 0x01DD | education# 触发器与效果
| 0x01DD | education_spending_state | 1 |
| 0x01DE | education_spending_nation# 触发器与效果
| 0x01DE | education_spending_nation | 1 |
| 0x01DF | trade_goods_in_state_state | 1# 触发器与效果
| 0x01DF | trade_goods_in_state_state | 1 |
| 0x01E0 | trade_goods_in_state_province | 1 |
| 0x01E1 | has# 触发器与效果
| 0x01E1 | has_flashpoint | 0 |
| 0x01E2 | flashpoint_tension |# 触发器与效果
| 0x01E2 | flashpoint_tension | 2 |
| 0x01E# 触发器与效果
| 0x01E3 | crisis_exist | 0 | Is there# 触发器与效果
| 0x01E3 | crisis_exist | 0 | Is there an active crisis right now? |
| 0x01E4 | is_liberation# 触发器与效果
| 0x01E4 | is_liberation_crisis | 0 |
| 0x01E5 | is_claim_cris# 触发器与效果
| 0x01E5 | is_claim_crisis | 0 |
| 0x01E6 | crisis_temperature | 2 |# 触发器与效果
| 0x01E6 | crisis_temperature | 2 | Crisis temperature compared with #1 (`float`) |
| 0x01E7 | involved# 触发器与效果
| 0x01E7 | involved_in_crisis_pop | 0 |
| 0x01E8 | involved_in# 触发器与效果
| 0x01E8 | involved_in_crisis_nation | 0 |
| 0x01E9 | rich_str# 触发器与效果
| 0x01E9 | rich_strata_life_needs_nation | 2# 触发器与效果
| 0x01E9 | rich_strata_life_needs_nation | 2 |
| 0x01EA | rich# 触发器与效果
| 0x01EA | rich_strata_life_needs_state | 2# 触发器与效果
| 0x01EA | rich_strata_life_needs_state | 2 |
| 0x01EB | rich# 触发器与效果
| 0x01EB | rich_strata_life_needs_province |# 触发器与效果
| 0x01EB | rich_strata_life_needs_province | 2 |
| 0x01EC | rich_strata_life_needs_pop |# 触发器与效果
| 0x01EC | rich_strata_life_needs_pop | 2 |
| 0x01ED |# 触发器与效果
| 0x01ED | rich_strata_everyday_needs_nation |# 触发器与效果
| 0x01ED | rich_strata_everyday_needs_nation | 2 |
| 0x01EE# 触发器与效果
| 0x01EE | rich_strata_everyday_needs_state |# 触发器与效果
| 0x01EE | rich_strata_everyday_needs_state | 2 |
| 0x01EF# 触发器与效果
| 0x01EF | rich_strata_everyday_needs_province# 触发器与效果
| 0x01EF | rich_strata_everyday_needs_province | 2 |
| 0x01F0 | rich_strata_everyday_needs# 触发器与效果
| 0x01F0 | rich_strata_everyday_needs_pop | 2 |
| 0x01F1 | rich_strata_luxury# 触发器与效果
| 0x01F1 | rich_strata_luxury_needs_nation | 2 |
| 0x01F2 | rich_strata_luxury_needs_state | 2 |
| 0x01F3 | rich# 触发器与效果
| 0x01F3 | rich_strata_luxury_needs_province | 2 |
| 0x01F# 触发器与效果
| 0x01F4 | rich_strata_luxury_needs# 触发器与效果
| 0x01F4 | rich_strata_luxury_needs_pop | 2 |
| 0x01F5 | middle_strata_life_ne# 触发器与效果
| 0x01F5 | middle_strata_life_needs_nation | 2 |
| 0x01F6 | middle_strata_l# 触发器与效果
| 0x01F6 | middle_strata_life_needs_state | 2 |
| 0x01F7 | middle_strata# 触发器与效果
| 0x01F7 | middle_strata_life_needs_province | 2 |
| 0x01F8 | middle# 触发器与效果
| 0x01F8 | middle_strata_life_needs_pop | 2# 触发器与效果
| 0x01F8 | middle_strata_life_needs_pop | 2 |
| 0x01F9 | middle_strata_everyday_needs_nation |# 触发器与效果
| 0x01F9 | middle_strata_everyday_needs_nation | 2 |
| 0x01FA# 触发器与效果
| 0x01FA | middle_strata_everyday_needs_state |# 触发器与效果
| 0x01FA | middle_strata_everyday_needs_state | 2 |
| 0x01FB# 触发器与效果
| 0x01FB | middle_strata_everyday_needs_province# 触发器与效果
| 0x01FB | middle_strata_everyday_needs_province | 2 |
| 0x01FC | middle_strata_everyday_needs_pop# 触发器与效果
| 0x01FC | middle_strata_everyday_needs_pop | 2 |
| 0x01FD | middle_strata_luxury_needs# 触发器与效果
| 0x01FD | middle_strata_luxury_needs_nation | 2 |
| 0x01FE | middle_strata_luxury# 触发器与效果
| 0x01FE | middle_strata_luxury_needs_state | 2 |
| 0x01FF | middle_strata_lux# 触发器与效果
| 0x01FF | middle_strata_luxury_needs_province | 2 |
| 0x0200 | middle_strata# 触发器与效果
| 0x0200 | middle_strata_luxury_needs_pop | 2 |
| 0x0201 | poor_str# 触发器与效果
| 0x0201 | poor_strata_life_needs_nation | 2# 触发器与效果
| 0x0201 | poor_strata_life_needs_nation | 2 |
| 0x0202 | poor# 触发器与效果
| 0x0202 | poor_strata_life_needs_state | 2# 触发器与效果
| 0x0202 | poor_strata_life_needs_state | 2 |
| 0x0203 | poor# 触发器与效果
| 0x0203 | poor_strata_life_needs_province |# 触发器与效果
| 0x0203 | poor_strata_life_needs_province | 2 |
| 0x0204 | poor_strata_life_needs_pop |# 触发器与效果
| 0x0204 | poor_strata_life_needs_pop | 2 |
| 0x0205 |# 触发器与效果
| 0x0205 | poor_strata_everyday_needs_nation |# 触发器与效果
| 0x0205 | poor_strata_everyday_needs_nation | 2 |
| 0x0206# 触发器与效果
| 0x0206 | poor_strata_everyday_needs_state |# 触发器与效果
| 0x0206 | poor_strata_everyday_needs_state | 2 |
| 0x0207# 触发器与效果
| 0x0207 | poor_strata_everyday_needs_province# 触发器与效果
| 0x0207 | poor_strata_everyday_needs_province | 2 |
| 0x020# 触发器与效果
| 0x0208 | poor_strata_everyday_needs_pop# 触发器与效果
| 0x0208 | poor_strata_everyday_needs_pop | 2 |
| 0x0209 | poor_strata_luxury_needs# 触发器与效果
| 0x0209 | poor_strata_luxury_needs_nation | 2 |
| 0x020A | poor_strata_luxury# 触发器与效果
| 0x020A | poor_strata_luxury_needs_state | 2 |
| 0x020B | poor_strata_lux# 触发器与效果
| 0x020B | poor_strata_luxury_needs_province | 2 |
| 0x020C | poor_strata# 触发器与效果
| 0x020C | poor_strata_luxury_needs_pop | 2 |
| 0x020D | diplomatic_in# 触发器与效果
| 0x020D | diplomatic_influence_tag | 2 |
| 0x020E | diplomatic_influence_this_nation | 1 |
| 0x020F | diplomatic_influence_this_province |# 触发器与效果
| 0x020F | diplomatic_influence_this_province | 1 |
| 0x0210 |# 触发器与效果
| 0x0210 | diplomatic_influence_from_nation | 1 |
| 0x0211 | diplomatic_in# 触发器与效果
| 0x0211 | diplomatic_influence_from_province | 1 |
| 0x0212 | pop_unemployment_nation | 3 |
| 0x0213 | pop_unemployment_state | 3# 触发器与效果
| 0x0213 | pop_unemployment_state | 3 |
| 0x0214 | pop# 触发器与效果
| 0x0214 | pop_unemployment_province | 3 |
| 0x0215 | pop_unemployment_pop# 触发器与效果
| 0x0215 | pop_unemployment_pop | 3 |
| 0x021# 触发器与效果
| 0x0216 | pop_unemployment_nation_this_pop | 2 |
| 0x0217# 触发器与效果
| 0x0217 | pop_unemployment_state_this_pop | 2# 触发器与效果
| 0x0217 | pop_unemployment_state_this_pop | 2 |
| 0x0218 | pop# 触发器与效果
| 0x0218 | pop_unemployment_province_this_pop | 2 |
| 0x0219 | relation_tag# 触发器与效果
| 0x0219 | relation_tag | 2 |
| 0x021A | relation_this_nation | 1 |
| 0x021B | relation_this# 触发器与效果
| 0x021B | relation_this_province | 1 |
| 0x021C | relation_from_nation |# 触发器与效果
| 0x021C | relation_from_nation | 1 |
| 0x021D |# 触发器与效果
| 0x021D | relation_from_province | 1 |
| 0x021E | check_variable |# 触发器与效果
| 0x021E | check_variable | 3 |
| 0x021F | upper_house | 3 |
| 0x0220 | unemployment_by_type_nation# 触发器与效果
| 0x0220 | unemployment_by_type_nation | 3 |
| 0x022# 触发器与效果
| 0x0221 | unemployment_by_type_state | 3 |
| 0x0222 | unemployment_by# 触发器与效果
| 0x0222 | unemployment_by_type_province | 3 |
| 0x0223 | unemployment_by_type_pop |# 触发器与效果
| 0x0223 | unemployment_by_type_pop | 3 |
| 0x0224# 触发器与效果
| 0x0224 | party_loyalty_nation_province_id | 3 |
| 0x0225 | party_loyalty_from_nation_prov# 触发器与效果
| 0x0225 | party_loyalty_from_nation_province_id | 3 |
| 0x0226 | party_loyalty_province# 触发器与效果
| 0x0226 | party_loyalty_province_province_id | 3 |
| 0x0227 | party_loyalty_from# 触发器与效果
| 0x0227 | party_loyalty_from_province_province_id | 3 |
| 0x0228 | party_loy# 触发器与效果
| 0x0228 | party_loyalty_nation_from_province | 2 |
| 0x0229 | party_lo# 触发器与效果
| 0x0229 | party_loyalty_from_nation_scope_province |# 触发器与效果
| 0x0229 | party_loyalty_from_nation_scope_province | 2 |
| 0x022A | can_build_in_province_railroad_no_limit# 触发器与效果
| 0x022A | can_build_in_province_railroad_no_limit_from_nation | 0 |
| 0x022B | can_build_in_province# 触发器与效果
| 0x022B | can_build_in_province_railroad_yes_limit_from_nation | 0 |
| 0x022C# 触发器与效果
| 0x022C | can_build_in_province_railroad_no# 触发器与效果
| 0x022C | can_build_in_province_railroad_no_limit_this_nation | 0 |
| 0x022D | can_build_in_prov# 触发器与效果
| 0x022D | can_build_in_province_railroad_yes_limit_this_nation | 0 |
| 0x022E | can_build_in_province_fort_no# 触发器与效果
| 0x022E | can_build_in_province_fort_no_limit_from_nation | 0 |
| 0x022F | can_build_in_prov# 触发器与效果
| 0x022F | can_build_in_province_fort_yes_limit_from_nation | 0 |
| 0x0230# 触发器与效果
| 0x0230 | can_build_in_province_fort_no_limit# 触发器与效果
| 0x0230 | can_build_in_province_fort_no_limit_this_nation | 0 |
| 0x0231 | can_build_in_province# 触发器与效果
| 0x0231 | can_build_in_province_fort_yes_limit_this_nation |# 触发器与效果
| 0x0231 | can_build_in_province_fort_yes_limit_this_nation | 0 |
| 0x0232 | can_build_in_province_naval_base_no_limit# 触发器与效果
| 0x0232 | can_build_in_province_naval_base_no_limit_from_nation | 0 |
| 0x0233 | can_build_in_province# 触发器与效果
| 0x0233 | can_build_in_province_naval_base_yes_limit_from_nation | 0 |
| 0x0234# 触发器与效果
| 0x0234 | can_build_in_province_naval_base_no# 触发器与效果
| 0x0234 | can_build_in_province_naval_base_no_limit_this_nation | 0 |
| 0x0235 | can_build_in_prov# 触发器与效果
| 0x0235 | can_build_in_province_naval_base_yes_limit_this_nation | 0 |
| 0x023# 触发器与效果
| 0x0236 | can_build_railway_in_capital# 触发器与效果
| 0x0236 | can_build_railway_in_capital_yes_whole_state_yes_limit |# 触发器与效果
| 0x0236 | can_build_railway_in_capital_yes_whole_state_yes_limit | 0 |
| 0x0237 | can_build_railway_in_capital_yes# 触发器与效果
| 0x0237 | can_build_railway_in_capital_yes_whole_state_no_limit | 0 |
| 0x0238 | can_build_r# 触发器与效果
| 0x0238 | can_build_railway_in_capital_no_whole_state_yes_limit | 0 |
| 0x0239 | can_build_railway_in# 触发器与效果
| 0x0239 | can_build_railway_in_capital_no_whole_state_no_limit |# 触发器与效果
| 0x0239 | can_build_railway_in_capital_no_whole_state_no_limit | 0 |
| 0x023A | can_build_fort_in_capital_yes_wh# 触发器与效果
| 0x023A | can_build_fort_in_capital_yes_whole_state_yes_limit | 0 |
| 0x023B | can_build_f# 触发器与效果
| 0x023B | can_build_fort_in_capital_yes_whole_state_no_limit | 0 |
| 0x023C | can_build_fort_in_capital# 触发器与效果
| 0x023C | can_build_fort_in_capital_no_whole_state_yes_limit | 0# 触发器与效果
| 0x023C | can_build_fort_in_capital_no_whole_state_yes_limit | 0 |
| 0x023D | can# 触发器与效果
| 0x023D | can_build_fort_in_capital_no_whole_state_no_limit | 0 |
| 0x023E | work_available_nation |# 触发器与效果
| 0x023E | work_available_nation | 1 |
| 0x023F |# 触发器与效果
| 0x023F | work_available_state | 1 |
| 0x0240 | work_available_province |# 触发器与效果
| 0x0240 | work_available_province | 1 |
| 0x0242# 触发器与效果
| 0x0242 | variable_ideology_name_nation |# 触发器与效果
| 0x0242 | variable_ideology_name_nation | 3 |
| 0x0243 | variable_ideology_name_state | 3 |
| 0x0244 | variable_# 触发器与效果
| 0x0244 | variable_ideology_name_province | 3 |
| 0x0245 | variable_ideology_name_pop | 3 |
| 0x0246 | variable_issue_name_nation | 3 |
| 0x024# 触发器与效果
| 0x0247 | variable_issue_name_state | 3 |
| 0x0248 | variable_issue# 触发器与效果
| 0x0248 | variable_issue_name_province | 3 |
| 0x0249 | variable_issue_name_pop |# 触发器与效果
| 0x0249 | variable_issue_name_pop | 3 |
| 0x024A# 触发器与效果
| 0x024A | variable_issue_group_name_nation | 2# 触发器与效果
| 0x024A | variable_issue_group_name_nation | 2 |
| 0x024B | variable# 触发器与效果
| 0x024B | variable_issue_group_name_state | 2 |
| 0x024C | variable_issue_group_name# 触发器与效果
| 0x024C | variable_issue_group_name_province | 2 |
| 0x024D | variable_issue_group_name_pop |# 触发器与效果
| 0x024D | variable_issue_group_name_pop | 2 |
| 0x024E# 触发器与效果
| 0x024E | variable_pop_type_name_nation | 3# 触发器与效果
| 0x024E | variable_pop_type_name_nation | 3 |
| 0x024F | variable# 触发器与效果
| 0x024F | variable_pop_type_name_state | 3 |
| 0x0250 | variable_pop_type_name# 触发器与效果
| 0x0250 | variable_pop_type_name_province | 3 |
| 0x0251 | variable_pop_type_name_pop |# 触发器与效果
| 0x0251 | variable_pop_type_name_pop | 3 |
| 0x0252# 触发器与效果
| 0x0252 | variable_good_name | 3 |
| 0x0253 | strata_middle |# 触发器与效果
| 0x0253 | strata_middle | 0 |
| 0x0254 | strata_poor | 0 |
| 0x0254 | party_loyalty_from# 触发器与效果
| 0x0254 | party_loyalty_from_province_scope_province | 2 |
| 0x0255 | can_build_factory# 触发器与效果
| 0x0255 | can_build_factory_nation | 0 |
| 0x0256 | can_build_factory_province | 0 |
| 0x0257# 触发器与效果
| 0x0257 | nationalvalue_pop | 1 |
| 0x0258 | nationalvalue_province# 触发器与效果
| 0x0258 | nationalvalue_province | 1 |
| 0x025# 触发器与效果
| 0x0259 | war_exhaustion_pop | 2# 触发器与效果
| 0x0259 | war_exhaustion_pop | 2 |
| 0x025A | has# 触发器与效果
| 0x025A | has_culture_core_province_this_pop | 0# 触发器与效果
| 0x025A | has_culture_core_province_this_pop | 0 |
| 0x025B | tag# 触发器与效果
| 0x025B | tag_pop | 1 |
| 0x025C | has_country_flag_pop | 1# 触发器与效果
| 0x025C | has_country_flag_pop | 1 |
| 0x025D | has# 触发器与效果
| 0x025D | has_country_flag_province | 1 |
| 0x025E | has_country_modifier_prov# 触发器与效果
| 0x025E | has_country_modifier_province | 1 |
| 0x025F | religion_nation | 1 |
| 0x0260 | religion_n# 触发器与效果
| 0x0260 | religion_nation_reb | 0 |
| 0x0261 | religion_nation_from_n# 触发器与效果
| 0x0261 | religion_nation_from_nation | 0 |
| 0x0262 | religion_nation_this_nation |# 触发器与效果
| 0x0262 | religion_nation_this_nation | 0 |
| 0x0263# 触发器与效果
| 0x0263 | religion_nation_this_state | 0 |
| 0x0264 | religion_n# 触发器与效果
| 0x0264 | religion_nation_this_province | 0 |
| 0x0265 | religion_nation_this_pop | 0 |
| 0x0266 | war_exhaustion_province |# 触发器与效果
| 0x0266 | war_exhaustion_province | 2 |
| 0x0267# 触发器与效果
| 0x0267 | is_greater_power_province | 0# 触发器与效果
| 0x0267 | is_greater_power_province | 0 |
| 0x0268 | is# 触发器与效果
| 0x0268 | is_cultural_union_pop_this_pop | 0 |
| 0x0269 | has_build# 触发器与效果
| 0x0269 | has_building_factory | 0 |
| 0x026A | has_building_state_from_prov# 触发器与效果
| 0x026A | has_building_state_from_province | 1 |
| 0x026B | has_building_factory_from_province | 0 |
| 0x026# 触发器与效果
| 0x026C | party_loyalty_generic | 2# 触发器与效果
| 0x026C | party_loyalty_generic | 2 |
| 0x026D | invention# 触发器与效果
| 0x026D | invention | 1 | Primary `dcon::n# 触发器与效果
| 0x026D | invention | 1 | Primary `dcon::nation_id` has invention #1 (`dcon# 触发器与效果
| 0x026D | invention | 1 | Primary `dcon::nation_id` has invention #1 (`dcon::invention_id`) |
| 0x026E | political_movement_from_reb# 触发器与效果
| 0x026E | political_movement_from_reb | 0 |
| 0x026F | social_movement_from_reb |# 触发器与效果
| 0x026F | social_movement_from_reb | 0 |
| 0x0270 | is_next_rreform_nation | 1# 触发器与效果
| 0x0270 | is_next_rreform_nation | 1 |
| 0x0271 | is# 触发器与效果
| 0x0271 | is_next_rreform_pop | 1 |
| 0x0272 | variable_reform# 触发器与效果
| 0x0272 | variable_reform_group_name_nation | 2 |
| 0x0273 | variable_reform_group_name_state | 2 |
| 0x0274 | variable_reform_group_name_prov# 触发器与效果
| 0x0274 | variable_reform_group_name_province | 2 |
| 0x0275 | variable_reform_group_name_pop | 2 |
| 0x0276# 触发器与效果
| 0x0276 | is_disarmed_pop | 0 | |
| 0x0277 | owned_by# 触发器与效果
| 0x0277 | owned_by_state_tag | 1 | |
| 0x0278 | owned_by_state_from_n# 触发器与效果
| 0x0278 | owned_by_state_from_nation | 0 | |
| 0x0279 | owned_by_state_this_nation# 触发器与效果
| 0x0279 | owned_by_state_this_nation | 0 | |
| 0x027A | owned_by_state_this_province | 0 | |
| 0x027# 触发器与效果
| 0x027B | owned_by_state_this_state | 0# 触发器与效果
| 0x027B | owned_by_state_this_state | 0 | |
| 0x027C |# 触发器与效果
| 0x027C | owned_by_state_this_pop | 0 | |
| 0x027D | units_in# 触发器与效果
| 0x027D | units_in_province_tag | 1 | |
| 0x027E | primary_culture_from_nation | 0 | |
| 0x027F | primary_culture_from_prov# 触发器与效果
| 0x027F | primary_culture_from_province | 0 | |
| 0x0280 | neighbour_this_province |# 触发器与效果
| 0x0280 | neighbour_this_province | 0 | |
| 0x0281 | neighbour_from_province | 0 | |
| 0x0282 | technology_prov# 触发器与效果
| 0x0282 | technology_province | 1 | |
| 0x0283 | invention_province | 1# 触发器与效果
| 0x0283 | invention_province | 1 | |
| 0x0284 | brigades_compare_province_this | 2 |# 触发器与效果
| 0x0284 | brigades_compare_province_this | 2 | |
| 0x0285 | brig# 触发器与效果
| 0x0285 | brigades_compare_province_from | 2 | |
| 0x0286 | is_accept# 触发器与效果
| 0x0286 | is_accepted_culture_nation_this_pop | 0# 触发器与效果
| 0x0286 | is_accepted_culture_nation_this_pop | 0 | |
| 0x0287 | is_accepted_culture_nation_this_nation# 触发器与效果
| 0x0287 | is_accepted_culture_nation_this_nation | 0 | |
| 0x0288 | is_accepted_culture_nation# 触发器与效果
| 0x0288 | is_accepted_culture_nation_this_state | 0 | |
| 0x0289 | is_accepted_culture# 触发器与效果
| 0x0289 | is_accepted_culture_nation_this_province | 0 | |
| 0x028A | is_accept# 触发器与效果
| 0x028A | is_accepted_culture_state_this_pop | 0 |# 触发器与效果
| 0x028A | is_accepted_culture_state_this_pop | 0 | |
| 0x028B | is# 触发器与效果
| 0x028B | is_accepted_culture_state_this_nation |# 触发器与效果
| 0x028B | is_accepted_culture_state_this_nation | 0 | |
| 0x028C | is_accepted_culture_state_this_state |# 触发器与效果
| 0x028C | is_accepted_culture_state_this_state | 0 | |
| 0x028# 触发器与效果
| 0x028D | is_accepted_culture_state_this_prov# 触发器与效果
| 0x028D | is_accepted_culture_state_this_province | 0 | |
| 0x028E | is_accepted_culture_prov# 触发器与效果
| 0x028E | is_accepted_culture_province_this_pop | 0 | |
| 0x028F | is_accepted_c# 触发器与效果
| 0x028F | is_accepted_culture_province_this_nation | 0 |# 触发器与效果
| 0x028F | is_accepted_culture_province_this_nation | 0 | |
| 0x0290 | is# 触发器与效果
| 0x0290 | is_accepted_culture_province_this_state |# 触发器与效果
| 0x0290 | is_accepted_culture_province_this_state | 0 | |
| 0x0291 | is_accepted_culture_province_this_prov# 触发器与效果
| 0x0291 | is_accepted_culture_province_this_province | 0 | |
| 0x0292 | is_accepted_culture_pop# 触发器与效果
| 0x0292 | is_accepted_culture_pop_this_pop | 0 | |
| 0x0293 | is_accepted_culture# 触发器与效果
| 0x0293 | is_accepted_culture_pop_this_nation | 0 | |
| 0x0294 | is_accepted_culture_pop_this_state | 0 | |
| 0x0295 | is_accept# 触发器与效果
| 0x0295 | is_accepted_culture_pop_this_province | 0# 触发器与效果
| 0x0295 | is_accepted_culture_pop_this_province | 0 | |
| 0x0296 | culture_group_province | 1 | |
| 0x0297 | culture_group_state# 触发器与效果
| 0x0297 | culture_group_state | 1 | |
| 0x0298 | have_core_in_nation_tag |# 触发器与效果
| 0x0298 | have_core_in_nation_tag | 1 | |
| 0x029# 触发器与效果
| 0x0299 | have_core_in_nation_this |# 触发器与效果
| 0x0299 | have_core_in_nation_this | 0 | |
| 0x029A# 触发器与效果
| 0x029A | have_core_in_nation_from | 0# 触发器与效果
| 0x029A | have_core_in_nation_from | 0 | |
| 0x029B | owns_province | 1 | |
| 0x029C | empty_state |# 触发器与效果
| 0x029C | empty_state | 0 | |
| 0x029D# 触发器与效果
| 0x029D | is_overseas_pop | 0 |# 触发器与效果
| 0x029D | is_overseas_pop | 0 | |
| 0x029E | primary# 触发器与效果
| 0x029E | primary_culture_pop | 1 | |
| 0x029F | plurality_pop |# 触发器与效果
| 0x029F | plurality_pop | 2 | |
| 0x02A# 触发器与效果
| 0x02A0 | is_overseas_state | 0# 触发器与效果
| 0x02A0 | is_overseas_state | 0 | |
| 0x02A1 | stronger_army_than_tag | 1 |# 触发器与效果
| 0x02A1 | stronger_army_than_tag | 1 | |
| 0x02A2 |# 触发器与效果
| 0x02A2 | region_state | 1 | |
| 0x02A3 | region_pop |# 触发器与效果
| 0x02A3 | region_pop | 1 | |
| 0x02A4 | owns_region | 1 | |
| 0x02A5 | is_core# 触发器与效果
| 0x02A5 | is_core_state_tag | 1 | |
| 0x02A6 | country_units_in_state# 触发器与效果
| 0x02A6 | country_units_in_state_from | 0 | |
| 0x02A7 | country_units_in_state_this# 触发器与效果
| 0x02A7 | country_units_in_state_this_nation | 0 | |
| 0x02A8 | country_units_in_state# 触发器与效果
| 0x02A8 | country_units_in_state_this_province | 0 | |
| 0x02A9 | country_units_in# 触发器与效果
| 0x02A9 | country_units_in_state_this_state | 0 | |
| 0x02AA | country_units_in_state# 触发器与效果
| 0x02AA | country_units_in_state_this_pop | 0 | |
| 0x02AB | country_units_in_state_tag | 1 | |
| 0x02AC | stronger_army_than_this_nation | 0 | |
| 0x02AD | stronger_army_than_this_state | 0 | |
| 0x02# 触发器与效果
| 0x02AE | stronger_army_than_this_province | 0 | |
| 0x02AF | stronger_army_than_this_pop |# 触发器与效果
| 0x02AF | stronger_army_than_this_pop | 0 | |
| 0x02B0 | stronger_army_than_from_nation | 0 | |
| 0x02B1 | stronger_army_than_from_province# 触发器与效果
| 0x02B1 | stronger_army_than_from_province | 0 | |
| 0x02B2 | flashpoint_tension_province# 触发器与效果
| 0x02B2 | flashpoint_tension_province | 2 | |
| 0x02B3 | is_colonial_pop |# 触发器与效果
| 0x02B3 | is_colonial_pop | 0 | |
| 0x02B# 触发器与效果
| 0x02B4 | has_country_flag_state | 1 |# 触发器与效果
| 0x02B4 | has_country_flag_state | 1 | |
| 0x02B5 | rich_tax_pop | 1 | |
| 0x02B6 | middle_tax_pop# 触发器与效果
| 0x02B6 | middle_tax_pop | 1 | |
| 0x02B7 | poor_tax_pop | 1# 触发器与效果
| 0x02B7 | poor_tax_pop | 1 | |
| 0x02B8# 触发器与效果
| 0x02B8 | is_core_pop_tag | 1 | |
| 0x02B9 | is# 触发器与效果
| 0x02B9 | is_core_boolean | 0 | |
| 0x02BA | is_core_state_this_n# 触发器与效果
| 0x02BA | is_core_state_this_nation | 0 | |
| 0x02BB | is_core_state_this_province# 触发器与效果
| 0x02BB | is_core_state_this_province | 0 | |
| 0x02BC | is_core_state_this_pop |# 触发器与效果
| 0x02BC | is_core_state_this_pop | 0 | |
| 0x02BD# 触发器与效果
| 0x02BD | is_core_state_from_nation | 0# 触发器与效果
| 0x02BD | is_core_state_from_nation | 0 | |
| 0x02BE | ruling_party_ideology_province | 1# 触发器与效果
| 0x02BE | ruling_party_ideology_province | 1 | |
| 0x02BF |# 触发器与效果
| 0x02BF | money_province | 2 | |
| 0x02C0 | is_our# 触发器与效果
| 0x02C0 | is_our_vassal_province_tag | 1 |# 触发器与效果
| 0x02C0 | is_our_vassal_province_tag | 1 | |
| 0x02C1 | is_our_vassal_province_from |# 触发器与效果
| 0x02C1 | is_our_vassal_province_from | 0 | |
| 0x02C2 | is_our_vassal_prov# 触发器与效果
| 0x02C2 | is_our_vassal_province_this_nation | 0 | |
| 0x02C3 | is_# 触发器与效果
| 0x02C3 | is_our_vassal_province_this_province | 0 | |
| 0x02C4 | is_our_vassal_prov# 触发器与效果
| 0x02C4 | is_our_vassal_province_this_state | 0 | |
| 0x02C5 | is_our# 触发器与效果
| 0x02C5 | is_our_vassal_province_this_pop | 0# 触发器与效果
| 0x02C5 | is_our_vassal_province_this_pop | 0 | |
| 0x02C6 | vassal_of_province_tag |# 触发器与效果
| 0x02C6 | vassal_of_province_tag | 1 | |
| 0x02C# 触发器与效果
| 0x02C7 | vassal_of_province_from |# 触发器与效果
| 0x02C7 | vassal_of_province_from | 0 | |
| 0x02C8 | vassal_of_province_this# 触发器与效果
| 0x02C8 | vassal_of_province_this_nation | 0 | |
| 0x02C9 | vassal_of# 触发器与效果
| 0x02C9 | vassal_of_province_this_province | 0 | |
| 0x02CA | vass# 触发器与效果
| 0x02CA | vassal_of_province_this_state | 0 |# 触发器与效果
| 0x02CA | vassal_of_province_this_state | 0 | |
| 0x02CB | v# 触发器与效果
| 0x02CB | vassal_of_province_this_pop | 0# 触发器与效果
| 0x02CB | vassal_of_province_this_pop | 0 | |
| 0x02CC | relation_this_pop | 1 | |
| 0x02CD | has_recently_l# 触发器与效果
| 0x02CD | has_recently_lost_war_pop | 0 | |
| 0x02CE | technology_pop |# 触发器与效果
| 0x02CE | technology_pop | 1 | |
| 0x02CF | invention_pop | 1 | |
| 0x02D0 | in_default# 触发器与效果
| 0x02D0 | in_default_bool | 0 | |
| 0x02D1 | is_state_capital_pop | 0 | |
| 0x02D2 | region_proper | 1# 触发器与效果
| 0x02D2 | region_proper | 1 | |
| 0x02D3# 触发器与效果
| 0x02D3 | region_proper_state | 1 | |
| 0x02D4 | region# 触发器与效果
| 0x02D4 | region_proper_pop | 1 | |
| 0x02D5 | owns_region_pro# 触发器与效果
| 0x02D5 | owns_region_proper | 1 | |
| 0x02D6 | pop_majority_relig# 触发器与效果
| 0x02D6 | pop_majority_religion_nation_this_nation | 0 |# 触发器与效果
| 0x02D6 | pop_majority_religion_nation_this_nation | 0 | |
| 0x02D7 | military_score_tag | 1 | |
| 0x02D8 | industrial_score_tag# 触发器与效果
| 0x02D8 | industrial_score_tag | 1 | |
| 0x02D9 | has_factories_nation |# 触发器与效果
| 0x02D9 | has_factories_nation | 0 | |
| 0x02DA | is_coastal_state | 0 |# 触发器与效果
| 0x02DA | is_coastal_state | 0 | |
| 0x02DB | has# 触发器与效果
| 0x02DB | has_building_bank | 0 | |
| 0x02DC | has_building_un# 触发器与效果
| 0x02DC | has_building_university | 0 | |
| 0x02DD | test | 1 | For# 触发器与效果
| 0x02DD | test | 1 | Forwards `THIS`, `FROM` and primary arguments# 触发器与效果
| 0x02DD | test | 1 | Forwards `THIS`, `FROM` and primary arguments, for more information see Scripted Triggers on the# 触发器与效果
| 0x02DD | test | 1 | Forwards `THIS`, `FROM` and primary arguments, for more information see Scripted Triggers on the modding extensions |
| 0x02E7 | is_colonial_crisis |# 触发器与效果
| 0x02E7 | is_colonial_crisis | 0 |
### 优化
Alice 在触发器代码解析后立即对其执行各种优化和简化：
-# 触发器与效果
- 对于作用域：
    - 如果# 触发器与效果
    - 如果作用域为空（没有可评估的内容）
        - 那么，消除整个作用域# 触发器与效果
        - 那么，消除整个作用域。
        - 否则，对于作用域# 触发器与效果
        - 否则，对于作用域中的每个成员：
            - 运行# 触发器与效果
            - 运行相同的优化/简化算法
    - 如果是# 触发器与效果
    - 如果是 `NOT` 作用域：
        - 反转 NOT 作用域内的所有内容（以消除整个作用域层）。
    - 如果作用域只有 1# 触发器与效果
    - 如果作用域只有 1 个成员且它是通用作用域：
        - 消除该作用域，仅保留# 触发器与效果
        - 消除该作用域，仅保留 1 个成员
    - 否则# 触发器与效果
    - 否则，如果它不是通用作用域（AND/OR）
        - 重新排列内部作用域（如果有）以取代外部 AND/OR 作用# 触发器与效果
        - 重新排列内部作用域（如果有）以取代外部 AND/OR 作用域
    - 如果它是通用作用域（AND/OR）
        - 折叠# 触发器与效果
        - 折叠它（将内部 AND/OR 重新排列到# 触发器与效果
        - 折叠它（将内部 AND/OR 重新排列到外部）
更多信息请参见 `trigger# 触发器与效果
更多信息请参见 `trigger::simplify_trigger`。
## 效果
### 什么是效果？
效果描述了将对游戏状态产生的"效果"——它们会修改游戏状态并允许执行各种不同的操作。与触发器不同，效果不是条件# 触发器与效果
效果描述了将对游戏状态产生的"效果"——它们会修改游戏状态并允许执行各种不同的操作。与触发器不同，效果不是条件性的，会按原样执行，但可以通过# 触发器与效果
效果描述了将对游戏状态产生的"效果"——它们会修改游戏状态并允许执行各种不同的操作。与触发器不同，效果不是条件性的，会按原样执行，但可以通过在其作用域中嵌入触发器来"限制"它们。熟悉模组制作的人可能会认识到 `# 触发器与效果
效果描述了将对游戏状态产生的"效果"——它们会修改游戏状态并允许执行各种不同的操作。与触发器不同，效果不是条件性的，会按原样执行，但可以通过在其作用域中嵌入触发器来"限制"它们。熟悉模组制作的人可能会认识到 `limit` 的用处。这允许条件在主# 触发器与效果
效果描述了将对游戏状态产生的"效果"——它们会修改游戏状态并允许执行各种不同的操作。与触发器不同，效果不是条件性的，会按原样执行，但可以通过在其作用域中嵌入触发器来"限制"它们。熟悉模组制作的人可能会认识到 `limit` 的用处。这允许条件在主触发器触发后仍然对效果起作用。
### 效果列表
| 代码# 触发器与效果
| 代码 | 名称 | 参数数量 | 备注# 触发器与效果
| 0x0000 | none | 0 |# 触发器与效果
| 0x0000 | none | 0 | Use for indicating no effect |
| 0x0001 | capital | 1 | ## 触发器与效果
| 0x0001 | capital | 1 | #1 is capital to move to |
| 0x0002 | add_core_tag |# 触发器与效果
| 0x0002 | add_core_tag | 1 | Adds all of the #1 argument (`# 触发器与效果
| 0x0002 | add_core_tag | 1 | Adds all of the #1 argument (`dcon::nation_id`) as cores |
| 0x0003 | add_core# 触发器与效果
| 0x0003 | add_core_int | 1 | ? |
| 0x0004 | add_core_this_nation# 触发器与效果
| 0x0004 | add_core_this_nation | 0 | Adds all of `THIS`# 触发器与效果
| 0x0004 | add_core_this_nation | 0 | Adds all of `THIS` (`dcon::nation_id`) as cores# 触发器与效果
| 0x0004 | add_core_this_nation | 0 | Adds all of `THIS` (`dcon::nation_id`) as cores |
| 0x0005 | add# 触发器与效果
| 0x0005 | add_core_this_province | 0 | Adds all# 触发器与效果
| 0x0005 | add_core_this_province | 0 | Adds all of `THIS` (`dcon::province_id# 触发器与效果
| 0x0005 | add_core_this_province | 0 | Adds all of `THIS` (`dcon::province_id`) as cores |
| 0x0006 | add_core_this_state | 0 |# 触发器与效果
| 0x0006 | add_core_this_state | 0 | Adds all of `THIS` (`dcon::# 触发器与效果
| 0x0006 | add_core_this_state | 0 | Adds all of `THIS` (`dcon::state_instance_id`) as cores |
| 0x0007 | add_core_this_pop | 0 | Adds `THIS` (`dcon# 触发器与效果
| 0x0007 | add_core_this_pop | 0 | Adds `THIS` (`dcon::pop_id` - location of) as cores# 触发器与效果
| 0x0007 | add_core_this_pop | 0 | Adds `THIS` (`dcon::pop_id` - location of) as cores |
| 0x0008 | add# 触发器与效果
| 0x0008 | add_core_from_province | 0 | Adds `# 触发器与效果
| 0x0008 | add_core_from_province | 0 | Adds `FROM` `dcon::province_id` as# 触发器与效果
| 0x0008 | add_core_from_province | 0 | Adds `FROM` `dcon::province_id` as cores |
| 0x0009 | add_core_from_nation | 0 | Adds# 触发器与效果
| 0x0009 | add_core_from_nation | 0 | Adds all of `FROM` `dcon::n# 触发器与效果
| 0x0009 | add_core_from_nation | 0 | Adds all of `FROM` `dcon::nation_id` as cores |
| 0x000A | add_core_reb |# 触发器与效果
| 0x000A | add_core_reb | 0 |
| 0x000B |# 触发器与效果
| 0x000B | remove_core_tag | 1 |
| 0x000C | remove_core_int |# 触发器与效果
| 0x000C | remove_core_int | 1 |
| 0x000D |# 触发器与效果
| 0x000D | remove_core_this_nation | 0 |
| 0x000E | remove_core_this# 触发器与效果
| 0x000E | remove_core_this_province | 0 |
| 0x000F | remove_core_this_state |# 触发器与效果
| 0x000F | remove_core_this_state | 0 |
| 0x0010 | remove_core_this_pop | 0 |
| 0x0011 | remove_core_from_prov# 触发器与效果
| 0x0011 | remove_core_from_province | 0 |
| 0x0012 | remove_core_from_nation |# 触发器与效果
| 0x0012 | remove_core_from_nation | 0 |
| 0x0013 |# 触发器与效果
| 0x0013 | remove_core_reb | 0 |
| 0x0014 | change_region_name_state# 触发器与效果
| 0x0014 | change_region_name_state | 1 | Sets name of the `d# 触发器与效果
| 0x0014 | change_region_name_state | 1 | Sets name of the `dcon::state_instance_id` to #1 (`# 触发器与效果
| 0x0014 | change_region_name_state | 1 | Sets name of the `dcon::state_instance_id` to #1 (`dcon::text_key_id`) |
| 0x0015 | change_region_name_prov# 触发器与效果
| 0x0015 | change_region_name_province | 1 | Sets name of the `# 触发器与效果
| 0x0015 | change_region_name_province | 1 | Sets name of the `dcon::province_id` to #1 (`# 触发器与效果
| 0x0015 | change_region_name_province | 1 | Sets name of the `dcon::province_id` to #1 (`dcon::text_key_id`) |
| 0x0016 | trade_goods |# 触发器与效果
| 0x0016 | trade_goods | 1 | Sets the RGO of the `d# 触发器与效果
| 0x0016 | trade_goods | 1 | Sets the RGO of the `dcon::province_id` to #1 (`d# 触发器与效果
| 0x0016 | trade_goods | 1 | Sets the RGO of the `dcon::province_id` to #1 (`dcon::commodity_id`) |
| 0x0017 | add_accepted_culture | 1 |
| 0x001# 触发器与效果
| 0x0018 | add_accepted_culture_union |# 触发器与效果
| 0x0018 | add_accepted_culture_union | 0 |
| 0x0019 |# 触发器与效果
| 0x0019 | primary_culture | 1 | Sets primary culture# 触发器与效果
| 0x0019 | primary_culture | 1 | Sets primary culture to #1 (`dcon::culture_id`)# 触发器与效果
| 0x0019 | primary_culture | 1 | Sets primary culture to #1 (`dcon::culture_id`) |
| 0x001A | primary# 触发器与效果
| 0x001A | primary_culture_this_nation | 0 |
| 0x001B | primary_culture# 触发器与效果
| 0x001B | primary_culture_this_state | 0 |
| 0x001C | primary_culture_this_province | 0 |
| 0x001D | primary_culture_this_pop | 0# 触发器与效果
| 0x001D | primary_culture_this_pop | 0 |
| 0x001E | primary_culture_from_nation | 0 |
| 0x001F | remove_accepted# 触发器与效果
| 0x001F | remove_accepted_culture | 1 |
| 0x0020 | life_rating | 1 |
| 0x0021 | religion |# 触发器与效果
| 0x0021 | religion | 1 |
| 0x0022# 触发器与效果
| 0x0022 | is_slave_state_yes | 0 |
| 0x0023 | is_slave# 触发器与效果
| 0x0023 | is_slave_pop_yes | 0 |
| 0x0024 | research_points | 1# 触发器与效果
| 0x0024 | research_points | 1 |
| 0x0025 | tech# 触发器与效果
| 0x0025 | tech_school | 1 |
| 0x0026 | government | 1 |
| 0x0027 | government_reb | 0 |
| 0x0028 | treasury | 2 |
| 0x0029 | war_exhaustion |# 触发器与效果
| 0x0029 | war_exhaustion | 2 |
| 0x002A# 触发器与效果
| 0x002A | prestige | 2 |
| 0x002B | change_tag | 1 |
| 0x002C | change_tag# 触发器与效果
| 0x002C | change_tag_culture | 0 |
| 0x002D | change_tag_no_core_switch | 1 |
| 0x002E# 触发器与效果
| 0x002E | change_tag_no_core_switch_culture |# 触发器与效果
| 0x002E | change_tag_no_core_switch_culture | 0 |
| 0x002F |# 触发器与效果
| 0x002F | set_country_flag | 1 |
| 0x0030 | clr_country_flag |# 触发器与效果
| 0x0030 | clr_country_flag | 1 |
| 0x0031 |# 触发器与效果
| 0x0031 | military_access | 1 |
| 0x0032 | military_access_this_nation | 0 |
| 0x0033# 触发器与效果
| 0x0033 | military_access_this_province | 0 |
| 0x0034 | military_access# 触发器与效果
| 0x0034 | military_access_from_nation | 0 |
| 0x0035 | military_access_from_province# 触发器与效果
| 0x0035 | military_access_from_province | 0 |
| 0x0036 | badboy | 2 |
| 0x0037 | secede_province | 1 |
| 0x0038 | secede_province_this_n# 触发器与效果
| 0x0038 | secede_province_this_nation | 0 |
| 0x0039 | secede_province_this_state# 触发器与效果
| 0x0039 | secede_province_this_state | 0 |
| 0x003A | secede_province_this_province | 0 |
| 0x003B | secede_province_this_pop |# 触发器与效果
| 0x003B | secede_province_this_pop | 0 |
| 0x003C | secede_province_from_nation | 0 |
| 0x003D# 触发器与效果
| 0x003D | secede_province_from_province |# 触发器与效果
| 0x003D | secede_province_from_province | 0 |
| 0x003E# 触发器与效果
| 0x003E | secede_province_reb |# 触发器与效果
| 0x003E | secede_province_reb | 0 |
| 0x003F |# 触发器与效果
| 0x003F | inherit | 1 |
| 0x0040 | inherit_this_nation | 0# 触发器与效果
| 0x0040 | inherit_this_nation | 0 |
| 0x0041 | inherit# 触发器与效果
| 0x0041 | inherit_this_state | 0 |
| 0x0042 | inherit_this_province |# 触发器与效果
| 0x0042 | inherit_this_province | 0 |
| 0x0043 |# 触发器与效果
| 0x0043 | inherit_this_pop | 0 |
| 0x0044 | inherit_from_nation | 0 |
| 0x0045# 触发器与效果
| 0x0045 | inherit_from_province | 0 |
| 0x0046 | annex_to |# 触发器与效果
| 0x0046 | annex_to | 1 |
| 0x0047 | annex_to_this_nation | 0 |
| 0x0048 | annex_to# 触发器与效果
| 0x0048 | annex_to_this_state | 0 |
| 0x0049 | annex_to_this_province |# 触发器与效果
| 0x0049 | annex_to_this_province | 0 |
| 0x004A# 触发器与效果
| 0x004A | annex_to_this_pop | 0 |
| 0x004B | annex_to_from# 触发器与效果
| 0x004B | annex_to_from_nation | 0 |
| 0x004C | annex_to_from_province | 0 |
| 0x004D# 触发器与效果
| 0x004D | release | 1 |
| 0x004E | release_this_nation |# 触发器与效果
| 0x004E | release_this_nation | 0 |
| 0x004F |# 触发器与效果
| 0x004F | release_this_state | 0 |
| 0x0050 | release_this_province |# 触发器与效果
| 0x0050 | release_this_province | 0 |
| 0x0051# 触发器与效果
| 0x0051 | release_this_pop | 0 |
| 0x0052 | release_from_nation# 触发器与效果
| 0x0052 | release_from_nation | 0 |
| 0x0053 | release_from_province | 0 |
| 0x0054 | change_controller# 触发器与效果
| 0x0054 | change_controller | 1 |
| 0x0055 | change_controller_this_nation | 0# 触发器与效果
| 0x0055 | change_controller_this_nation | 0 |
| 0x0056 | change# 触发器与效果
| 0x0056 | change_controller_this_province | 0 |
| 0x0057 | change_controller_from_n# 触发器与效果
| 0x0057 | change_controller_from_nation | 0 |
| 0x0058 | change_controller_from_province |# 触发器与效果
| 0x0058 | change_controller_from_province | 0 |
| 0x0059 | infrastructure | 1 |
| 0x005A | money | 2 |
| 0x005B | leadership | 1# 触发器与效果
| 0x005B | leadership | 1 |
| 0x005C | create# 触发器与效果
| 0x005C | create_vassal | 1 |
| 0x005D | create_vassal_this# 触发器与效果
| 0x005D | create_vassal_this_nation | 0 |
| 0x005E | create_vassal_this_prov# 触发器与效果
| 0x005E | create_vassal_this_province | 0 |
| 0x005F | create_vassal_from_nation | 0 |
| 0x0060 | create_vassal_from_province |# 触发器与效果
| 0x0060 | create_vassal_from_province | 0 |
| 0x0061# 触发器与效果
| 0x0061 | end_military_access | 1 |
| 0x0062 | end_military# 触发器与效果
| 0x0062 | end_military_access_this_nation | 0 |
| 0x0063 | end_military_access_this_province | 0 |
| 0x0064 | end_military_access_from# 触发器与效果
| 0x0064 | end_military_access_from_nation | 0 |
| 0x0065 | end_military_access_from_province | 0 |
| 0x0066 | leave_alliance | 1 |
| 0x0067 | leave_all# 触发器与效果
| 0x0067 | leave_alliance_this_nation | 0 |
| 0x0068 | leave_alliance_this# 触发器与效果
| 0x0068 | leave_alliance_this_province | 0 |
| 0x0069 | leave_alliance_from_nation | 0 |
| 0x006A | leave_alliance_from_province |# 触发器与效果
| 0x006A | leave_alliance_from_province | 0 |
| 0x006B |# 触发器与效果
| 0x006B | end_war | 1 |
| 0x006C | end_war_this_n# 触发器与效果
| 0x006C | end_war_this_nation | 0 |
| 0x006D | end_war_this_province | 0 |
| 0x006E# 触发器与效果
| 0x006E | end_war_from_nation | 0# 触发器与效果
| 0x006E | end_war_from_nation | 0 |
| 0x006F | end# 触发器与效果
| 0x006F | end_war_from_province | 0 |
| 0x0070 | enable_ide# 触发器与效果
| 0x0070 | enable_ideology | 1 |
| 0x0071 | ruling_party_ideology |# 触发器与效果
| 0x0071 | ruling_party_ideology | 1 |
| 0x0072 | plurality | 2 |
| 0x0073 | remove_province_modifier | 1# 触发器与效果
| 0x0073 | remove_province_modifier | 1 |
| 0x0074 | remove# 触发器与效果
| 0x0074 | remove_country_modifier | 1 |
| 0x0075 | create_alliance | 1# 触发器与效果
| 0x0075 | create_alliance | 1 |
| 0x0076 | create# 触发器与效果
| 0x0076 | create_alliance_this_nation | 0 |
| 0x0077 | create_alliance# 触发器与效果
| 0x0077 | create_alliance_this_province | 0 |
| 0x0078 | create_alliance_from_nation | 0 |
| 0x0079 | create_alliance_from_province |# 触发器与效果
| 0x0079 | create_alliance_from_province | 0 |
| 0x007A# 触发器与效果
| 0x007A | release_vassal | 1 |
| 0x007B | release_vass# 触发器与效果
| 0x007B | release_vassal_this_nation | 0 |
| 0x007C | release_vassal_this_province | 0 |
| 0x007D | release_vassal_from# 触发器与效果
| 0x007D | release_vassal_from_nation | 0 |
| 0x007E | release_vassal_from_province | 0 |
| 0x007F | release_vassal_reb |# 触发器与效果
| 0x007F | release_vassal_reb | 0 |
| 0x0080# 触发器与效果
| 0x0080 | release_vassal_random | 0 |
| 0x0081 | change_prov# 触发器与效果
| 0x0081 | change_province_name | 1 |
| 0x0082 | enable_canal | 1# 触发器与效果
| 0x0082 | enable_canal | 1 |
| 0x0083 | set# 触发器与效果
| 0x0083 | set_global_flag | 1 |
| 0x0084 | clr_global_flag | 1# 触发器与效果
| 0x0084 | clr_global_flag | 1 |
| 0x0085 | national# 触发器与效果
| 0x0085 | nationalvalue_province | 1 |
| 0x0086 | nationalvalue_nation |# 触发器与效果
| 0x0086 | nationalvalue_nation | 1 |
| 0x0087# 触发器与效果
| 0x0087 | civilized_yes | 0 |
| 0x0088 | civilized_no# 触发器与效果
| 0x0088 | civilized_no | 0 |
| 0x0089 | is_slave_state_no | 0 |
| 0x008A | is_slave# 触发器与效果
| 0x008A | is_slave_pop_no | 0 |
| 0x008B | election | 0 |
| 0x008C | social_reform# 触发器与效果
| 0x008C | social_reform | 1 |
| 0x008# 触发器与效果
| 0x008D | political_reform | 1 |
| 0x008E | add_tax_relative# 触发器与效果
| 0x008E | add_tax_relative_income | 2 |
| 0x008F | neutrality | 0 |
| 0x0090 | reduce_pop | 2 |
| 0x0091# 触发器与效果
| 0x0091 | move_pop | 1 |
| 0x0092 | pop_type | 1# 触发器与效果
| 0x0092 | pop_type | 1 |
| 0x0093 | years# 触发器与效果
| 0x0093 | years_of_research | 2 |
| 0x0094 | prestige_factor_positive |# 触发器与效果
| 0x0094 | prestige_factor_positive | 2 |
| 0x0095 |# 触发器与效果
| 0x0095 | prestige_factor_negative | 2 |
| 0x0096 | military_reform |# 触发器与效果
| 0x0096 | military_reform | 1 |
| 0x0097 |# 触发器与效果
| 0x0097 | economic_reform | 1 |
| 0x0098 | remove_random_military_re# 触发器与效果
| 0x0098 | remove_random_military_reforms | 1 |
| 0x0099 | remove_random_economic_reforms |# 触发器与效果
| 0x0099 | remove_random_economic_reforms | 1 |
| 0x009A# 触发器与效果
| 0x009A | add_crime | 1 |
| 0x009B | add_crime_none# 触发器与效果
| 0x009B | add_crime_none | 0 |
| 0x009C | nationalize | 0 |
| 0x009D | build_factory_in_capital_state | 1 |
| 0x009E | activate_technology |# 触发器与效果
| 0x009E | activate_technology | 1 |
| 0x009F |# 触发器与效果
| 0x009F | great_wars_enabled_yes | 0 |
| 0x00A0 | great# 触发器与效果
| 0x00A0 | great_wars_enabled_no | 0 |
| 0x00A1 | world_wars# 触发器与效果
| 0x00A1 | world_wars_enabled_yes | 0 |
| 0x00A2 | world_wars_enabled# 触发器与效果
| 0x00A2 | world_wars_enabled_no | 0 |
| 0x00A3 | assimilate_province |# 触发器与效果
| 0x00A3 | assimilate_province | 0 |
| 0x00A4 | assimilate_pop | 0 |
| 0x00A5 | literacy |# 触发器与效果
| 0x00A5 | literacy | 2 |
| 0x00A6# 触发器与效果
| 0x00A6 | add_crisis_interest | 0# 触发器与效果
| 0x00A6 | add_crisis_interest | 0 |
| 0x00A7 |# 触发器与效果
| 0x00A7 | flashpoint_tension | 2 |
| 0x00A8 | add_cris# 触发器与效果
| 0x00A8 | add_crisis_temperature | 2 |
| 0x00A9 | consciousness | 2 |
| 0x00AA | militancy# 触发器与效果
| 0x00AA | militancy | 2 |
| 0x00AB | rgo_size | 1 |
| 0x00AC | fort |# 触发器与效果
| 0x00AC | fort | 1 |
| 0x00AD |# 触发器与效果
| 0x00AD | naval_base | 1 |
| 0x00AE | trigger_revolt_nation |# 触发器与效果
| 0x00AE | trigger_revolt_nation | 4 |
| 0x00AF# 触发器与效果
| 0x00AF | trigger_revolt_state | 4 |
| 0x00B0 | trigger_rev# 触发器与效果
| 0x00B0 | trigger_revolt_province | 4 |
| 0x00B1 | diplomatic_influence |# 触发器与效果
| 0x00B1 | diplomatic_influence | 2 |
| 0x00B# 触发器与效果
| 0x00B2 | diplomatic_influence_this_nation |# 触发器与效果
| 0x00B2 | diplomatic_influence_this_nation | 1 |
| 0x00B3# 触发器与效果
| 0x00B3 | diplomatic_influence_this_province | 1# 触发器与效果
| 0x00B3 | diplomatic_influence_this_province | 1 |
| 0x00B4 |# 触发器与效果
| 0x00B4 | diplomatic_influence_from_nation | 1 |
| 0x00B5 | diplomatic# 触发器与效果
| 0x00B5 | diplomatic_influence_from_province | 1 |
| 0x00B6 | relation |# 触发器与效果
| 0x00B6 | relation | 2 |
| 0x00B7 | relation_this_nation | 1 |
| 0x00B8 | relation# 触发器与效果
| 0x00B8 | relation_this_province | 1 |
| 0x00B9 | relation_from_nation# 触发器与效果
| 0x00B9 | relation_from_nation | 1 |
| 0x00BA | relation_from_province | 1 |
| 0x00BB | add_prov# 触发器与效果
| 0x00BB | add_province_modifier | 2 |
| 0x00BC | add_province_modifier_no_duration | 1 |
| 0x00BD | add_country_modifier | 2 |
| 0x00BE | add_country_modifier# 触发器与效果
| 0x00BE | add_country_modifier_no_duration | 1 |
| 0x00BF | casus_belli_tag |# 触发器与效果
| 0x00BF | casus_belli_tag | 3 |
| 0x00C# 触发器与效果
| 0x00C0 | casus_belli_int | 3# 触发器与效果
| 0x00C0 | casus_belli_int | 3 |
| 0x00C1 | casus_belli_this_nation | 2# 触发器与效果
| 0x00C1 | casus_belli_this_nation | 2 |
| 0x00C2 |# 触发器与效果
| 0x00C2 | casus_belli_this_state | 2 |
| 0x00C3 | cas# 触发器与效果
| 0x00C3 | casus_belli_this_province | 2 |
| 0x00C4 | cas# 触发器与效果
| 0x00C4 | casus_belli_this_pop | 2 |
| 0x00C5 | casus# 触发器与效果
| 0x00C5 | casus_belli_from_nation | 2 |
| 0x00C6 | casus# 触发器与效果
| 0x00C6 | casus_belli_from_province | 2 |
| 0x00C7 | add_c# 触发器与效果
| 0x00C7 | add_casus_belli_tag | 3 |
| 0x00C8 | add_casus# 触发器与效果
| 0x00C8 | add_casus_belli_int | 3 |
| 0x00C9 | add_casus_b# 触发器与效果
| 0x00C9 | add_casus_belli_this_nation | 2 |
| 0x00CA | add_casus_b# 触发器与效果
| 0x00CA | add_casus_belli_this_state | 2 |
| 0x00CB | add_casus_belli_this_province | 2 |
| 0x00CC | add_casus_belli# 触发器与效果
| 0x00CC | add_casus_belli_this_pop | 2 |
| 0x00CD | add_casus_belli_from_nation | 2 |
| 0x00CE | add_casus_belli_from# 触发器与效果
| 0x00CE | add_casus_belli_from_province | 2 |
| 0x00CF | remove_casus_belli_tag# 触发器与效果
| 0x00CF | remove_casus_belli_tag | 2 |
| 0x00D0 | remove_casus_belli_int |# 触发器与效果
| 0x00D0 | remove_casus_belli_int | 2 |
| 0x00D# 触发器与效果
| 0x00D1 | remove_casus_belli_this_nation | 1 |
| 0x00D2 | remove_casus_belli_this_state# 触发器与效果
| 0x00D2 | remove_casus_belli_this_state | 1 |
| 0x00D3 | remove_casus_belli_this_prov# 触发器与效果
| 0x00D3 | remove_casus_belli_this_province | 1 |
| 0x00D4 | remove_casus_belli_this_pop | 1 |
| 0x00D5 | remove_casus_belli_from# 触发器与效果
| 0x00D5 | remove_casus_belli_from_nation | 1 |
| 0x00D6 | remove_casus_belli# 触发器与效果
| 0x00D6 | remove_casus_belli_from_province | 1 |
| 0x00D7 | this_remove_casus# 触发器与效果
| 0x00D7 | this_remove_casus_belli_tag | 2 |
| 0x00D8 | this_remove_casus# 触发器与效果
| 0x00D8 | this_remove_casus_belli_int | 2 |
| 0x00D9 | this_remove_casus# 触发器与效果
| 0x00D9 | this_remove_casus_belli_this_nation | 1 |
| 0x00DA | this_remove_c# 触发器与效果
| 0x00DA | this_remove_casus_belli_this_state | 1 |
| 0x00DB | this_remove_c# 触发器与效果
| 0x00DB | this_remove_casus_belli_this_province | 1 |
| 0x00DC | this_remove# 触发器与效果
| 0x00DC | this_remove_casus_belli_this_pop | 1 |
| 0x00DD | this_remove# 触发器与效果
| 0x00DD | this_remove_casus_belli_from_nation | 1# 触发器与效果
| 0x00DD | this_remove_casus_belli_from_nation | 1 |
| 0x00DE | this# 触发器与效果
| 0x00DE | this_remove_casus_belli_from_province |# 触发器与效果
| 0x00DE | this_remove_casus_belli_from_province | 1 |
| 0x00DF | war_tag | 7 |
| 0x00E0 | war_this_nation |# 触发器与效果
| 0x00E0 | war_this_nation | 6 |
| 0x00E1 | war_this_state | 6 |
| 0x00E2 | war_this# 触发器与效果
| 0x00E2 | war_this_province | 6 |
| 0x00E3 | war_this_pop |# 触发器与效果
| 0x00E3 | war_this_pop | 6 |
| 0x00E4 | war_from_nation | 6 |
| 0x00E5 | war_from# 触发器与效果
| 0x00E5 | war_from_province | 6 |
| 0x00E6 | war_no_ally_tag# 触发器与效果
| 0x00E6 | war_no_ally_tag | 7 |
| 0x00E7 | war_no_ally_this_nation | 6 |
| 0x00E8 | war_no_ally_this_state |# 触发器与效果
| 0x00E8 | war_no_ally_this_state | 6 |
| 0x00E9 | war_no_ally_this_province |# 触发器与效果
| 0x00E9 | war_no_ally_this_province | 6 |
| 0x00EA# 触发器与效果
| 0x00EA | war_no_ally_this_pop | 6# 触发器与效果
| 0x00EA | war_no_ally_this_pop | 6 |
| 0x00EB | war# 触发器与效果
| 0x00EB | war_no_ally_from_nation | 6 |
| 0x00EC | war_no# 触发器与效果
| 0x00EC | war_no_ally_from_province | 6 |
| 0x00ED | country_event_this# 触发器与效果
| 0x00ED | country_event_this_nation | 2 |
| 0x00EE | country_event_immediate_this_nation | 1 |
| 0x00EF | province_event_this_nation |# 触发器与效果
| 0x00EF | province_event_this_nation | 2 |
| 0x00F0# 触发器与效果
| 0x00F0 | province_event_immediate_this_nation |# 触发器与效果
| 0x00F0 | province_event_immediate_this_nation | 1 |
| 0x00F1 | sub_unit_int | 2 |
| 0x00F2 | sub_unit_this# 触发器与效果
| 0x00F2 | sub_unit_this | 1 |
| 0x00F3 | sub_unit_from | 1 |
| 0x00F4 | sub# 触发器与效果
| 0x00F4 | sub_unit_current | 1 |
| 0x00F5 | set_variable | 3# 触发器与效果
| 0x00F5 | set_variable | 3 |
| 0x00F6 | change_variable | 3 |
| 0x00F7 | ideology | 3 |
| 0x00F8 | upper# 触发器与效果
| 0x00F8 | upper_house | 3 |
| 0x00F9 | scaled_militancy_issue | 3 |
| 0x00FA | scaled_militancy_ideology |# 触发器与效果
| 0x00FA | scaled_militancy_ideology | 3 |
| 0x00FB# 触发器与效果
| 0x00FB | scaled_militancy_unemployment | 2# 触发器与效果
| 0x00FB | scaled_militancy_unemployment | 2 |
| 0x00FC | scaled# 触发器与效果
| 0x00FC | scaled_consciousness_issue | 3 |
| 0x00FD | scaled_consciousness# 触发器与效果
| 0x00FD | scaled_consciousness_ideology | 3 |
| 0x00FE | scaled_consciousness_un# 触发器与效果
| 0x00FE | scaled_consciousness_unemployment | 2 |
| 0x00FF | define_general | 3 |
| 0x0100 | define_admir# 触发器与效果
| 0x0100 | define_admiral | 3 |
| 0x0101 | dominant_issue | 3 |
| 0x0102 | add_war_goal | 1 |
| 0x0103 | move_issue_percentage_nation |# 触发器与效果
| 0x0103 | move_issue_percentage_nation | 4 |
| 0x0104 |# 触发器与效果
| 0x0104 | move_issue_percentage_state | 4 |
| 0x0105 | move_issue_percentage_prov# 触发器与效果
| 0x0105 | move_issue_percentage_province | 4 |
| 0x0106 | move_issue_percentage_pop | 4# 触发器与效果
| 0x0106 | move_issue_percentage_pop | 4 |
| 0x0107 | party# 触发器与效果
| 0x0107 | party_loyalty | 3 |
| 0x0108 | party_loyalty_prov# 触发器与效果
| 0x0108 | party_loyalty_province | 2 |
| 0x0109 | variable_tech_name_no |# 触发器与效果
| 0x0109 | variable_tech_name_no | 1 |
| 0x010A |# 触发器与效果
| 0x010A | variable_invention_name_yes | 1 |
| 0x010B | build_r# 触发器与效果
| 0x010B | build_railway_in_capital_yes_whole_state_yes_limit | 0 |
| 0x010C | build_railway_in# 触发器与效果
| 0x010C | build_railway_in_capital_yes_whole_state_no_limit | 0 |
| 0x010D# 触发器与效果
| 0x010D | build_railway_in_capital_no_wh# 触发器与效果
| 0x010D | build_railway_in_capital_no_whole_state_yes_limit | 0 |
| 0x010E | build_rail# 触发器与效果
| 0x010E | build_railway_in_capital_no_whole_state_no_limit | 0 |
| 0x010# 触发器与效果
| 0x010F | build_fort_in_capital_yes# 触发器与效果
| 0x010F | build_fort_in_capital_yes_whole_state_yes_limit | 0 |
| 0x0110 | build_f# 触发器与效果
| 0x0110 | build_fort_in_capital_yes_whole_state_no_limit | 0 |
| 0x0111 | build_fort_in_capital_no# 触发器与效果
| 0x0111 | build_fort_in_capital_no_whole_state_yes_limit | 0 |
| 0x0112 | build_f# 触发器与效果
| 0x0112 | build_fort_in_capital_no_whole_state_no_limit | 0 |
| 0x0113 | relation_reb | 1 |
| 0x0114 | variable_tech# 触发器与效果
| 0x0114 | variable_tech_name_yes | 1 |
| 0x0115 | variable_good_name |# 触发器与效果
| 0x0115 | variable_good_name | 3 |
| 0x0116 |# 触发器与效果
| 0x0116 | set_country_flag_province | 1 |
| 0x0117 | add_country_modifier# 触发器与效果
| 0x0117 | add_country_modifier_province | 2 |
| 0x0118 | add_country_modifier_province_no_duration | 1 |
| 0x0119 | dominant_issue_nation | 3# 触发器与效果
| 0x0119 | dominant_issue_nation | 3 |
| 0x011A | relation# 触发器与效果
| 0x011A | relation_province | 2 |
| 0x011B | relation_province_this_nation# 触发器与效果
| 0x011B | relation_province_this_nation | 1 |
| 0x011C | relation_province_this_province |# 触发器与效果
| 0x011C | relation_province_this_province | 1 |
| 0x011D | relation_province_from_nation | 1 |
| 0x011E | relation_prov# 触发器与效果
| 0x011E | relation_province_from_province | 1 |
| 0x011F | relation_province_re# 触发器与效果
| 0x011F | relation_province_reb | 1 |
| 0x0120 | scaled_militancy_nation_issue | 3 |
| 0x0121 | scaled_militancy_nation_ide# 触发器与效果
| 0x0121 | scaled_militancy_nation_ideology | 3 |
| 0x0122 | scaled_militancy_nation_un# 触发器与效果
| 0x0122 | scaled_militancy_nation_unemployment | 2 |
| 0x0123 | scaled_consciousness_nation_issue# 触发器与效果
| 0x0123 | scaled_consciousness_nation_issue | 3 |
| 0x012# 触发器与效果
| 0x0124 | scaled_consciousness_nation_ideology | 3 |
| 0x0125 | scaled_consciousness_nation_un# 触发器与效果
| 0x0125 | scaled_consciousness_nation_unemployment | 2 |
| 0x0126 | scaled_militancy_nation_issue# 触发器与效果
| 0x0126 | scaled_militancy_nation_issue | 3 |
| 0x0127 | scaled_militancy_nation_ideology | 3 |
| 0x0128 | scaled_militancy_nation_un# 触发器与效果
| 0x0128 | scaled_militancy_nation_unemployment | 2 |
| 0x0129 | scaled_consciousness_nation_issue# 触发器与效果
| 0x0129 | scaled_consciousness_nation_issue | 3 |
| 0x012A | scaled_consciousness_nation_ideology | 3 |
| 0x012B | scaled_consciousness_nation_un# 触发器与效果
| 0x012B | scaled_consciousness_nation_unemployment | 2 |
| 0x012C | scaled_militancy_nation_issue# 触发器与效果
| 0x012C | scaled_militancy_nation_issue | 3 |
| 0x012D | scaled_militancy_nation_ideology | 3 |
| 0x012E | scaled_militancy_nation_un# 触发器与效果
| 0x012E | scaled_militancy_nation_unemployment | 2 |
| 0x012F | scaled_consciousness_nation_issue | 3 |
| 0x0130 | scaled_consciousness_nation_ideology | 3 |
| 0x0131 | scaled_consciousness_nation_un# 触发器与效果
| 0x0131 | scaled_consciousness_nation_unemployment | 2 |
| 0x0132 | variable_good_name_province |# 触发器与效果
| 0x0132 | variable_good_name_province | 3 |
| 0x0133 |# 触发器与效果
| 0x0133 | treasury_province | 2 |
| 0x0134 | country_event_this_state |# 触发器与效果
| 0x0134 | country_event_this_state | 2 |
| 0x0135# 触发器与效果
| 0x0135 | country_event_immediate_this_state | 1# 触发器与效果
| 0x0135 | country_event_immediate_this_state | 1 |
| 0x0136 | province# 触发器与效果
| 0x0136 | province_event_this_state | 2 |
| 0x0137 | province_event_immediate_this# 触发器与效果
| 0x0137 | province_event_immediate_this_state | 1 |
| 0x0138 | country_event_this_province |# 触发器与效果
| 0x0138 | country_event_this_province | 2 |
| 0x0139 |# 触发器与效果
| 0x0139 | country_event_immediate_this_province | 1# 触发器与效果
| 0x0139 | country_event_immediate_this_province | 1 |
| 0x013A | province# 触发器与效果
| 0x013A | province_event_this_province | 2 |
| 0x013B | province_event_immediate# 触发器与效果
| 0x013B | province_event_immediate_this_province | 1 |
| 0x013C | country_event_this_pop |# 触发器与效果
| 0x013C | country_event_this_pop | 2 |
| 0x013D# 触发器与效果
| 0x013D | country_event_immediate_this_pop | 1# 触发器与效果
| 0x013D | country_event_immediate_this_pop | 1 |
| 0x013E | province# 触发器与效果
| 0x013E | province_event_this_pop | 2 |
| 0x013F | province_event_immediate_this# 触发器与效果
| 0x013F | province_event_immediate_this_pop | 1 |
| 0x0140 | country_event_province_this_nation# 触发器与效果
| 0x0140 | country_event_province_this_nation | 2 |
| 0x0141 | country_event_immediate_province_this_nation | 1 |
| 0x0142 | country_event_province_this_state |# 触发器与效果
| 0x0142 | country_event_province_this_state | 2 |
| 0x0143# 触发器与效果
| 0x0143 | country_event_immediate_province_this_state | 1 |
| 0x0144 | country_event_province_this_province |# 触发器与效果
| 0x0144 | country_event_province_this_province | 2 |
| 0x0145 |# 触发器与效果
| 0x0145 | country_event_immediate_province_this_province | 1 |
| 0x0146# 触发器与效果
| 0x0146 | country_event_province_this_pop | 2# 触发器与效果
| 0x0146 | country_event_province_this_pop | 2 |
| 0x0147 | country# 触发器与效果
| 0x0147 | country_event_immediate_province_this_pop | 1# 触发器与效果
| 0x0147 | country_event_immediate_province_this_pop | 1 |
| 0x0148 | activate# 触发器与效果
| 0x0148 | activate_invention | 1 |
| 0x0149 | variable_invention_name_no | 1 |
| 0x0014# 触发器与效果
| 0x0014A | add_core_tag_state | 1 |
| 0x0014B | remove# 触发器与效果
| 0x0014B | remove_core_tag_state | 1 |
| 0x014C | secede_province# 触发器与效果
| 0x014C | secede_province_state | 1 |
| 0x014D | assimilate_state | 0 |
| 0x014E | add_core# 触发器与效果
| 0x014E | add_core_state_this_nation | 0 | |
| 0x014F | add_core_state# 触发器与效果
| 0x014F | add_core_state_this_province | 0 | |
| 0x0150 | add_core_state_this_state | 0 | |
| 0x0151 | add_core_state_this_pop |# 触发器与效果
| 0x0151 | add_core_state_this_pop | 0 | |
| 0x0152 | add_core_state_from_province |# 触发器与效果
| 0x0152 | add_core_state_from_province | 0 | |
| 0x0153 | add_core_state_from_nation | 0# 触发器与效果
| 0x0153 | add_core_state_from_nation | 0 | |
| 0x0154 |# 触发器与效果
| 0x0154 | add_core_state_reb | 0 | |
| 0x0155 | add_prov# 触发器与效果
| 0x0155 | add_province_modifier_state | 2 | |
| 0x0156 | add_province_modifier# 触发器与效果
| 0x0156 | add_province_modifier_state_no_duration | 1 | |
| 0x0157 | remove_core_state_this# 触发器与效果
| 0x0157 | remove_core_state_this_nation | 0 | |
| 0x0158 | remove_core_state_this_province | 0 | |
| 0x0159 | remove_core_state_this_state | 0 | |
| 0x015A | remove_core_state_this_pop | 0# 触发器与效果
| 0x015A | remove_core_state_this_pop | 0 | |
| 0x015B | remove_core_state_from_province | 0 |# 触发器与效果
| 0x015B | remove_core_state_from_province | 0 | |
| 0x015C | remove# 触发器与效果
| 0x015C | remove_core_state_from_nation | 0 | |
| 0x015D | remove_core# 触发器与效果
| 0x015D | remove_core_state_reb | 0 | |
| 0x015E | remove_province_modifier# 触发器与效果
| 0x015E | remove_province_modifier_state | 1 | |
| 0x015F | life_rating_state | 1# 触发器与效果
| 0x015F | life_rating_state | 1 | |
| 0x0160 | secede_province_state_this_nation |# 触发器与效果
| 0x0160 | secede_province_state_this_nation | 0 | |
| 0x016# 触发器与效果
| 0x0161 | secede_province_state_this_state# 触发器与效果
| 0x0161 | secede_province_state_this_state | 0 | |
| 0x0162 | secede_province_state_this# 触发器与效果
| 0x0162 | secede_province_state_this_province | 0 | |
| 0x0163 | secede_province# 触发器与效果
| 0x0163 | secede_province_state_this_pop | 0 | |
| 0x0164 | secede_prov# 触发器与效果
| 0x0164 | secede_province_state_from_nation | 0 | |
| 0x0165 | seced# 触发器与效果
| 0x0165 | secede_province_state_from_province | 0# 触发器与效果
| 0x0165 | secede_province_state_from_province | 0 | |
| 0x0166 | secede_province_state_reb |# 触发器与效果
| 0x0166 | secede_province_state_reb | 0 | |
| 0x0167# 触发器与效果
| 0x0167 | infrastructure_state | 1 | |
| 0x0168 | fort_state |# 触发器与效果
| 0x0168 | fort_state | 1 | |
| 0x0169# 触发器与效果
| 0x0169 | naval_base_state | 1 | |
| 0x016A | is_slave_prov# 触发器与效果
| 0x016A | is_slave_province_yes | 0 | |
| 0x016B | is_slave_province# 触发器与效果
| 0x016B | is_slave_province_no | 0 | |
| 0x016C | change_controller_state | 1# 触发器与效果
| 0x016C | change_controller_state | 1 | |
| 0x016D | change_controller_state_this_nation | 0 |# 触发器与效果
| 0x016D | change_controller_state_this_nation | 0 | |
| 0x016E | change# 触发器与效果
| 0x016E | change_controller_state_this_province | 0 | |
| 0x016F | change_controller# 触发器与效果
| 0x016F | change_controller_state_from_nation | 0 | |
| 0x0170 | change_controller_state# 触发器与效果
| 0x0170 | change_controller_state_from_province | 0 | |
| 0x0171 | reduce_pop_province# 触发器与效果
| 0x0171 | reduce_pop_province | 2 | |
| 0x0172 | reduce_pop_state | 2 |# 触发器与效果
| 0x0172 | reduce_pop_state | 2 | |
| 0x0173 | reduce# 触发器与效果
| 0x0173 | reduce_pop_nation | 2 | |
| 0x0174 | consciousness_province |# 触发器与效果
| 0x0174 | consciousness_province | 2 | |
| 0x0175 | consciousness_state | 2 | |
| 0x0176 | consciousness_nation# 触发器与效果
| 0x0176 | consciousness_nation | 2 | |
| 0x0177 | militancy_province | 2# 触发器与效果
| 0x0177 | militancy_province | 2 | |
| 0x0178 |# 触发器与效果
| 0x0178 | militancy_state | 2 | |
| 0x0179 | militancy_nation# 触发器与效果
| 0x0179 | militancy_nation | 2 | |
| 0x017A | remove_core_tag_nation |# 触发器与效果
| 0x017A | remove_core_tag_nation | 1 | |
| 0x017B | remove_core_nation_this_nation |# 触发器与效果
| 0x017B | remove_core_nation_this_nation | 0 | |
| 0x017C# 触发器与效果
| 0x017C | remove_core_nation_this_province |# 触发器与效果
| 0x017C | remove_core_nation_this_province | 0 | |
| 0x017D | remove_core_nation_this_state | 0# 触发器与效果
| 0x017D | remove_core_nation_this_state | 0 | |
| 0x017E |# 触发器与效果
| 0x017E | remove_core_nation_this_pop | 0 |# 触发器与效果
| 0x017E | remove_core_nation_this_pop | 0 | |
| 0x017F | remove# 触发器与效果
| 0x017F | remove_core_nation_from_province | 0 | |
| 0x0180 | remove# 触发器与效果
| 0x0180 | remove_core_nation_from_nation | 0 |# 触发器与效果
| 0x0180 | remove_core_nation_from_nation | 0 | |
| 0x0181 | remove# 触发器与效果
| 0x0181 | remove_core_nation_reb | 0 | |
| 0x0182 | set_country# 触发器与效果
| 0x0182 | set_country_flag_pop | 1 | |
| 0x0183 | social_reform_province# 触发器与效果
| 0x0183 | social_reform_province | 1 | |
| 0x0184 | political_reform_province |# 触发器与效果
| 0x0184 | political_reform_province | 1 | |
| 0x0185 | flashpoint_tension_province | 2# 触发器与效果
| 0x0185 | flashpoint_tension_province | 2 | |
| 0x0186 |# 触发器与效果
| 0x0186 | release_vassal_province | 1 |# 触发器与效果
| 0x0186 | release_vassal_province | 1 | |
| 0x0187 | release# 触发器与效果
| 0x0187 | release_vassal_province_this_nation |# 触发器与效果
| 0x0187 | release_vassal_province_this_nation | 0 | |
| 0x0188 | release_vassal_province_this_province# 触发器与效果
| 0x0188 | release_vassal_province_this_province | 0 | |
| 0x0189 | release_vassal_province_from# 触发器与效果
| 0x0189 | release_vassal_province_from_nation | 0 | |
| 0x018A | release_vassal_prov# 触发器与效果
| 0x018A | release_vassal_province_from_province | 0 | |
| 0x018B | release_vass# 触发器与效果
| 0x018B | release_vassal_province_reb | 0 | |
| 0x018C | release_v# 触发器与效果
| 0x018C | release_vassal_province_random | 0 | |
| 0x018D | build_bank# 触发器与效果
| 0x018D | build_bank_in_capital_yes_whole_state_yes_limit | 0 | |
| 0x018E | build_bank_in_capital_y# 触发器与效果
| 0x018E | build_bank_in_capital_yes_whole_state_no_limit | 0 |# 触发器与效果
| 0x018E | build_bank_in_capital_yes_whole_state_no_limit | 0 | |
| 0x018F | build# 触发器与效果
| 0x018F | build_bank_in_capital_no_whole_state_yes_limit | 0 | |
| 0x0190 | build_bank_in_capital_no# 触发器与效果
| 0x0190 | build_bank_in_capital_no_whole_state_no_limit | 0 | |
| 0x0191 | build_un# 触发器与效果
| 0x0191 | build_university_in_capital_yes_whole_state_yes_limit | 0 | |
| 0x0192 | build_university_in_cap# 触发器与效果
| 0x0192 | build_university_in_capital_yes_whole_state_no_limit |# 触发器与效果
| 0x0192 | build_university_in_capital_yes_whole_state_no_limit | 0 | |
| 0x0193 | build_university_in_capital_no_whole# 触发器与效果
| 0x0193 | build_university_in_capital_no_whole_state_yes_limit | 0 | |
| 0x0194 | build_university# 触发器与效果
| 0x0194 | build_university_in_capital_no_whole_state_no_limit | 0 | |
| 0x0195 | bank | 1 | |
| 0x0196 | bank_state |# 触发器与效果
| 0x0196 | bank_state | 1 | |
| 0x0197# 触发器与效果
| 0x0197 | university | 1 | |
| 0x0198 | university_state | 1# 触发器与效果
| 0x0198 | university_state | 1 | |
| 0x0199 | kill_leader | 2 | |
| 0x019A | annex_to_null_nation# 触发器与效果
| 0x019A | annex_to_null_nation | 0 | |
| 0x019B | annex_to_null_province |# 触发器与效果
| 0x019B | annex_to_null_province | 0 | |
| 0x019C# 触发器与效果
| 0x019C | add_truce_tag | 2 | |
| 0x019D | add_tr# 触发器与效果
| 0x019D | add_truce_this_nation | 1 | |
| 0x019E | add_truce# 触发器与效果
| 0x019E | add_truce_this_state | 1 | |
| 0x019F | add_truce_this_prov# 触发器与效果
| 0x019F | add_truce_this_province | 1 | |
| 0x01A0 | add_truce_this_pop# 触发器与效果
| 0x01A0 | add_truce_this_pop | 1 | |
| 0x01A1 | add_truce_from_nation# 触发器与效果
| 0x01A1 | add_truce_from_nation | 1 | |
| 0x01A2 | add_truce_from_province# 触发器与效果
| 0x01A2 | add_truce_from_province | 1 | |
| 0x01A3 | call_allies | 0# 触发器与效果
| 0x01A3 | call_allies | 0 | |
| 0x01A4# 触发器与效果
| 0x01A4 | ruling_party_this | 0 | |
| 0x01A5 | ruling_party# 触发器与效果
| 0x01A5 | ruling_party_from | 0 | |
| 0x01A6 | add_accepted_culture_this | 0 | |
| 0x01A7 | add_accepted_culture